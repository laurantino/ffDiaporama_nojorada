...
QBrush *GetGradientBrush(QRectF Rect, int BrushType, int GradientOrientation, QString ColorD, QString ColorF, QString ColorIntermed, double Intermediate) 
{
   ...
   switch (GradientOrientation)
   {
      ...
      case GRADIENTORIENTATION_UP:          
         //Gradient = QLinearGradient(QPointF(Rect.x() + Rect.width() / 2, Rect.y()), QPointF(Rect.x() + Rect.width() / 2, Rect.y() + Rect.height()));
         Gradient = QLinearGradient(0,0, Rect.y(), Rect.y()+Rect.height());
         break;          // Up
   ...
   if (BrushType == BRUSHTYPE_GRADIENT3) 
      Gradient.setColorAt(Intermediate, QColor(ColorIntermed));
...
cBackgroundObject::cBackgroundObject(QString FileName,cBaseAppConfig *AppConfig) 
{
   ...
   IsValide = false;
   ModifDateTime = QFileInfo(FileName).FILECREATIONDATE();
...
void cBackgroundList::ScanDisk(QString Path,cBaseAppConfig *ApplicationConfig) 
{
   ...
   {
      if (Files[i].isFile() && ((QString(Files[i].suffix()).toLower() == QStringLiteral("jpg") )||(QString(Files[i].suffix()).toLower()==QStringLiteral("png")))) 
      {
         QString FileName = QFileInfo(Files[i]).absoluteFilePath();
         if (QFileInfo(QString(FileName)).isFile()) 
            List.append(cBackgroundObject(Files[i].absoluteFilePath(),ApplicationConfig));
      } 
...
int cBackgroundList::SearchImage(QString NameToFind) 
{
   int Ret = -1;
   int j = 0;
   while ( j < List.count() && Ret==-1) 
      if (List[j].Name == NameToFind) 
         Ret=j; 
      else j++;
   if (Ret==-1) 
      return 0; // If not found : switch to first background
   return Ret;
}
...
bool cBaseBrushDefinition::assignChanges(const cBaseBrushDefinition& rhs, const cBaseBrushDefinition& savedBrush)
{
   if (rhs.BrushType != BrushType) 
      return false;
   if( savedBrush.PatternType!= rhs.PatternType) 
      PatternType = rhs.PatternType;                
   if (savedBrush.GradientOrientation != rhs.GradientOrientation) 
      GradientOrientation = rhs.GradientOrientation;
   if (savedBrush.ColorD != rhs.ColorD) 
      ColorD = rhs.ColorD;
   if (savedBrush.ColorF != rhs.ColorF) 
      ColorF = rhs.ColorF;
   if (savedBrush.ColorIntermed != rhs.ColorIntermed) 
      ColorIntermed = rhs.ColorIntermed;
   if (savedBrush.Intermediate != rhs.Intermediate) 
      Intermediate = rhs.Intermediate;
   if (savedBrush.BrushImage != rhs.BrushImage) 
      BrushImage = rhs.BrushImage;
   return true;
}
