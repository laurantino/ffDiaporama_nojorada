...
//*****************************************************************************************************************************************
    ...
    QIcon GetIconForFileOrDir(QString FileName,int IconIndex) 
    {
       ...
       if (RetIcon.isNull()) 
            TOLOG(LOGMSG_CRITICAL, QString("Loading icon nbr %1 From %2 Error").arg(IconIndex).arg(FileName));
       return RetIcon;
    }
...
void cCustomIcon::LoadIcons(QString FileName) 
{
   ...
   if (Icon.isNull()) 
         TOLOG(LOGMSG_CRITICAL, QString("Loading %1 Error").arg(FileName))
   else
   {
...
void cCustomIcon::LoadIconsFromIMG(QString FileName) 
{
    ...
    if (Icon16.isNull())  
      TOLOG(LOGMSG_CRITICAL,QString("Loading img/MediaIcons/16x16/%1 Error").arg(FileName));
    Icon100.load(":/img/MediaIcons/100x100/"+FileName);   
    if (Icon100.isNull()) 
       TOLOG(LOGMSG_CRITICAL,QString("Loading img/MediaIcons/100x100/%1 Error").arg(FileName));
}
...
void cCustomIcon::LoadIconsFromLinux(QString LinuxPath,QString FileName) 
{
    ...
    if (Icon16.isNull())  
       TOLOG(LOGMSG_CRITICAL,QString("Loading %116x16/%2 Error").arg(LinuxPath).arg(FileName));
    Icon100=QImage(LinuxPath+"128x128/"+FileName);      
    if (Icon100.isNull()) 
       TOLOG(LOGMSG_CRITICAL,QString("Loading %1128x128/%2 Error").arg(LinuxPath).arg(FileName));
    if (!Icon100.isNull()) 
       Icon100 = Icon100.scaledToHeight(100,Qt::SmoothTransformation);
}
...
void cCustomIcon::LoadIcons(QIcon Icon) 
{
   ...
   {
      if (Icon100.height() > Icon100.width()) 
         Icon100 = Icon100.scaledToHeight(100, Qt::SmoothTransformation);
      else 
         Icon100 = Icon100.scaledToWidth(100, Qt::SmoothTransformation);
   }
}
