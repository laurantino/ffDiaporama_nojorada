...
class cFrameList 
{
   QList<cDiaporamaObjectInfo *> frames;            // List of rendered frames
public:
   cFrameList();
   ~cFrameList();

   void ClearList();                // Clear the list (make delete of each frame)
   cDiaporamaObjectInfo *DetachFirstFrame();        // Detach the first image of the list (do not make delete)
   cDiaporamaObjectInfo *GetFirstFrame();           // Retreve a link to the first frame in the list
   cDiaporamaObjectInfo *GetLastFrame();            // Retreve a link to the last frame in the list
   void AppendFrame(cDiaporamaObjectInfo *Frame);   // Append an image to the end of the list
...
class QCustomRuler : public QSlider 
{
   ...
   public:
      ...
      bool EditStartEnd;       // if true, the ruler display 2 buttons to edit start and end position
