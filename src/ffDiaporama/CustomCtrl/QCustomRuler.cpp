...
//====================================================================================================================
cDiaporamaObjectInfo *cFrameList::DetachFirstFrame() 
{
   if (frames.count() > 0) 
      return frames.takeFirst(); 
   return NULL;
}
...
//====================================================================================================================
cDiaporamaObjectInfo *cFrameList::GetFirstFrame() 
{
   if (!frames.isEmpty() ) 
      return frames.first(); 
   return NULL;
...
//====================================================================================================================
cDiaporamaObjectInfo *cFrameList::GetLastFrame() 
{
   if (!frames.isEmpty() ) 
      return frames.last(); 
   return NULL;
...
//====================================================================================================================
void cFrameList::AppendFrame(cDiaporamaObjectInfo *Frame) 
{
   frames.append(Frame);
}
...
void QCustomRuler::PrepareSoundWave() 
{
   ...
   for (int i = 0; i < Peak.count(); i++) 
   {
      ...
      Painter.drawLine(QPointF(i,Peak[i]*Height),QPointF(i,-Peak[i]*Height));
      Painter.setPen(GreenPen); 
      Painter.drawLine(QPointF(i,Moyenne[i]*Height),QPointF(i,-Moyenne[i]*Height));
   }
   Painter.restore();
...
void QCustomRuler::mousePressEvent(QMouseEvent *ev) 
{
   if (ev->button() == Qt::LeftButton) 
   {
      ...
      { 
         ...
         emit PositionChangeByUser(StartPos);
         setValue(StartPos);
      } 
      else if (EditStartEnd && (ev->y() > DecalY+16) && (ev->x() >= EndX-ThumbWidth) && (ev->x() <= EndX+ThumbWidth))    // It's on the end thumb
      {
         ...
         emit PositionChangeByUser();
         emit PositionChangeByUser(EndPos);
         setValue(EndPos);
      } 
      ...
      else if ((ev->x() >= TAQUET_SIZE-ThumbWidth/2) && (ev->x() <= this->width()-TAQUET_SIZE+ThumbWidth/2))   // It's on the ruler so set new value
      {
         TrackingMode = TrackingMode_None;
         emit PositionChangeByUser();
         emit PositionChangeByUser(int(double((ev->x()-TAQUET_SIZE)/double(this->width()-TAQUET_SIZE*2))*double(maximum())));
...
void QCustomRuler::mouseReleaseEvent(QMouseEvent *ev) 
{
   if (isSliderDown()) 
   {
      ...
      update();
      emit PositionChangeByUser();
      emit PositionChangeByUser(value());
   } 
   else 
...
void QCustomRuler::paintEvent(QPaintEvent *) 
{
   ...
   int  WidthTick = Painter.fontMetrics().QT_FONT_WIDTH("00:00");
   ...
   // Now : Draw line and texts of the ruler
   ...
   while ((TAQUET_SIZE*2+Cur*SizeTick+(WidthTick/2)) < Width) 
   {
      ...
      // Draw text
      if (Ech == QLatin1String("H") || Ech == QLatin1String("10M")) 
         Text = QString("%1:%2").arg(TimeHour,2,10,QChar('0')).arg(TimeMin,2,10,QChar('0'));
      else 
         Text = QString("%1:%2").arg(TimeMin,2,10,QChar('0')).arg(TimeSec,2,10,QChar('0'));
      ...
      if ((WidthTick < (SizeTick/3)) && (TAQUET_SIZE+(Cur+0.5)*SizeTick+(WidthTick/2) < Width)) 
         Painter.drawText(TAQUET_SIZE+(Cur+0.5)*SizeTick-(WidthTick/2),32-1,Text);
