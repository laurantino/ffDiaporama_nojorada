                     ...
                     QString ButtonYesText,QString ButtonNoText) 
{
   ...
   while (mask <= QMessageBox::LastButton) 
   {
      ...
      if ((sb == QMessageBox::Yes) && (!ButtonYesText.isEmpty())) 
         button->setText(ButtonYesText);
      if ((sb == QMessageBox::No) && (!ButtonNoText.isEmpty())) 
         button->setText(ButtonNoText);
...
QCustomDialog::~QCustomDialog() 
{
   if (!HelpFile.isEmpty() && ApplicationConfig->WikiFollowInterface) 
      ApplicationConfig->PopupHelp->RestorePreviousHelpFile();
...
void QCustomDialog::InitDialog() 
{
   ...
   if (UndoBt) 
      UndoBt->setEnabled(UndoDataList.count() > 0);
...
{
   if ((event->modifiers() == Qt::NoModifier) && (event->key() == Qt::Key_F1) && (HelpBt) && (!HelpFile.isEmpty())) 
      help();
   else 
   ...
   if (ApplicationConfig->SettingsTable->GetIntAndTextValue(objectName(),&TypeWindowState,&WindowStateString)) 
   {
      ...
      else 
      {
         ...
         else if (TypeWindowState == TypeWindowState_withsplitterpos) 
         {
            ...
            DlgWSP.LoadFromXML(domDocument.documentElement());
            if (DlgWSP.IsInit) 
               DlgWSP.ApplyToWindow(this,Splitter);
         }
...
void QCustomDialog::DoPartialUndo() 
{
   ...
   } else 
   {
      ...
      if (Data.FocusWindow != NULL) 
         Data.FocusWindow->setFocus();
   }
   if (UndoBt) 
      UndoBt->setEnabled(UndoDataList.count() > 0);
}
...
void QCustomDialog::SetCBIndex(QComboBox *CB,int ItemData) 
{
   ...
   for (int i = 0; i < CB->count(); i++) 
      if (CB->itemData(i).toInt() == ItemData) 
      {
