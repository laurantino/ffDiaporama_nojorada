...
#define ObjectBackground_Ruler  Qt::darkGray
...
{
   if (ParentTable->columnCount() == 0) 
      return;
   //AUTOTIMER(a,"QCustomThumbItemDelegate::paint");
   QApplication::setOverrideCursor(QCursor(Qt::WaitCursor));
   Painter->save();

   int ItemIndex = index.row()*ParentTable->columnCount()+index.column();

   if (ItemIndex >= ParentTable->Diaporama->slides.count()) 
   {
   ...
   cDiaporamaObject *Object = ParentTable->Diaporama->slides[ItemIndex];
   cDiaporamaObject *PreviousObject = ItemIndex > 0 ? ParentTable->Diaporama->slides[ItemIndex - 1] : NULL;
   cDiaporamaObject *NextObject = ItemIndex < ParentTable->Diaporama->slides.count() - 1 ? ParentTable->Diaporama->slides[ItemIndex + 1] : NULL;

   int ThumbWidth = ParentTable->columnWidth(0);
   int ThumbHeight = ParentTable->rowHeight(0);
   bool IsTransition = ((Object->TransitionFamily != 0) || (Object->TransitionSubType != 0));
   QPointF Table[10];
   ...
   int FontFactor = ((ParentTable->ApplicationConfig->TimelineHeight-TIMELINEMINHEIGHT)/20)*10;
   ...
   Painter->fillRect(QRect(0, (ThumbHeight / 4 - 16) / 2, ThumbWidth, 16), QColor(ObjectBackground_Ruler));

   // Draw thumb
   ParentTable->Diaporama->PrepareBackground(ItemIndex, QSize(BackThumbWidth, BackThumbHeight), Painter, TransitionSize + 3, 2);
   ...
   if (Object->BackgroundType) 
      DrawThumbnailsBox(2, (ThumbHeight / 4 - 32) / 2 - 1, 32, 32, Painter, IsTransition ? IconList.GetIcon(TRANSITIONFAMILY_BASE, 1) : IconList.GetIcon(TRANSITIONFAMILY_BASE, 0));
   ...
   //==========================================================================================================================
   // Transition box
   DrawThumbnailsBox(2,ThumbHeight/4+2-1,32,32,Painter,IsTransition?IconList.GetIcon(Object->TransitionFamily,Object->TransitionSubType):IconList.GetIcon(TRANSITIONFAMILY_BASE,0));
   ...
   if (PreviousSoundVolume == -1) 
      PreviousSoundVolume = 1;
   ...
   QString TransitionDuration = QTime(0, 0, 0, 0).addMSecs(Object->CachedTransitDuration).toString("s.z");
   //if (TransitionDuration[0]=='0')             
   //   TransitionDuration=TransitionDuration.right(TransitionDuration.length()-1);   // Cut first 0
   while (TransitionDuration.endsWith("0"))    
      TransitionDuration = TransitionDuration.left(TransitionDuration.length()-1);
   while (TransitionDuration.endsWith("."))    
      TransitionDuration = TransitionDuration.left(TransitionDuration.length()-1);
   ...
   {
      Pen.setColor(ObjectBackground_Ruler);
      Pen.setWidth(1);
      Pen.setStyle(Qt::SolidLine);
      Painter->setPen(Pen);
      Painter->setBrush(QBrush(QColor(ObjectBackground_Ruler)));
   ...
   if (!HaveSound) 
   {
      ...
      if (Object->ObjectComposition.compositionObjects.count() > 1)   
         Painter->drawImage(TransitionSize+3+8,                  2-1+ThumbHeight/4+8,    QImage(ICON_BLOCKPRESENCE));    // Add mark if multiple block
      if (Object->shotList.count() > 1)                     
         Painter->drawImage(TransitionSize + 3 + NewThumbWidth - 32, 2 - 1 + ThumbHeight / 4 + 8, QImage(ICON_SHOTPRESENCE));     // Add mark if multiple shot
      if (HaveFilter)
         Painter->drawImage(TransitionSize+3+NewThumbWidth-32,   2-1+ThumbHeight/4+32,   QImage(ICON_HAVEFILTER));       // Add mark if at least one block have filter
   ...
   else 
   {
      ...
      Painter->drawRect(TransitionSize+3-2,ThumbHeight/4+1-2,NewThumbWidth+4,VideoThumbHeight+4);
      ...
      if (Object->ObjectComposition.compositionObjects.count() > 1)
         Painter->drawImage(TransitionSize + 3 + BarWidth + 8, 2 - 1 + ThumbHeight / 4 + 8, QImage(ICON_BLOCKPRESENCE));    // Add mark if multiple block
      if (Object->shotList.count() > 1)
         Painter->drawImage(TransitionSize + 3 + BarWidth + VideoThumbWidth - 32, 2 - 1 + ThumbHeight / 4 + 8, QImage(ICON_SHOTPRESENCE));     // Add mark if multiple shot
      if (HaveFilter)
         Painter->drawImage(TransitionSize + 3 + BarWidth + VideoThumbWidth - 32, 2 - 1 + ThumbHeight / 4 + 32, QImage(ICON_HAVEFILTER));       // Add mark if at least one block have filter
   ...
   // Chapter TAG
   if (Object->StartNewChapter)
      Painter->drawImage(TransitionSize+3,2,QImage(ICON_CHAPTER));
   ...
   cMusicObject    *CurMusic           = Object->pDiaporama->GetMusicObject(ItemIndex,StartPosition,&CurrentCountObjet,&OwnerObjectMusic);

   int             PrevCountObjet      = 0;
   int64_t         PrevStartPosition   = 0;
   cMusicObject    *PrevMusic          = PreviousObject?Object->pDiaporama->GetMusicObject(ItemIndex-1,PrevStartPosition,&PrevCountObjet):NULL;
   ...
   else if (PrevEndMusicTransit) // Previous music end during the entering transition
   {
      if ((PrevCountObjet & 1)==1) 
      {
         Painter->setBrush(QBrush(QColor(FirstMusic_Color)));
   ...
   if (CurMusic && CurMusic != PrevMusic) 
   {
      ...
      if (OwnerObjectMusic != ItemIndex) 
         Painter->drawImage(TransitionSize-1-24,ThumbHeight-4-24+1,QImage(ICON_CONTINUEPLAYLIST));
   ...
   if (Object->MusicPause) 
      Painter->drawImage((ThumbWidth-24-TransitionSize)/2+TransitionSize,ThumbHeight-5-24,QImage(ICON_PLAYERPAUSE));
   // Draw separated line
   Pen.setWidth(1);
   ...
   if ((ParentTable->CursorPosValide) || (ParentTable->DragItemDest == ParentTable->DragItemSource))
   {
      ...
      {             
         ...
         {
            if (ItemIndex==ParentTable->DragItemDest)
               DrawDragBefore = true;
            if ((ItemIndex==ParentTable->DragItemDest-1)&&(ItemIndex==ParentTable->Diaporama->slides.count()-1))
               DrawDragAfter = true;
         }
...
void cCustomSlideTable::dragEnterEvent(QDragEnterEvent *event) 
{
   ...
   if (event->mimeData()->hasUrls())
   {
      ...
      if ((MusicFileList.count() > 0) && (FileList.count() == 0) && (Diaporama->slides.count() > 0))
      {
...
void cCustomSlideTable::dragLeaveEvent(QDragLeaveEvent *event) 
{
   if (IsDragOn == DRAGMODE_EXTERNALADD_SLIDE)
   {
      ...
      if ((NbrX > 0) && (DragItemDest >= 0))
      {
         ...
         DragItemDest = -1;
         if (ToUse < Diaporama->slides.count())  
            update(model()->index(ToUse / NbrX, ToUse - (ToUse / NbrX)*NbrX));
         else
            update(model()->index((ToUse - 1) / NbrX, (ToUse - 1) - ((ToUse - 1) / NbrX)*NbrX));
      }
      IsDragOn = DRAGMODE_NOACTION;
   }
   event->accept();
...
void cCustomSlideTable::dropEvent(QDropEvent *event) 
{
   ...
   if (event->mimeData()->hasUrls())
   {
      ...
      //{
         ...
         //info.setFile(fName);                                            // information about file
         //if (info.isFile()) 
         //   ((MainWindow *)ApplicationConfig->TopLevelWindow)->FileList.append(fName);    // append file
         //if(urlList[i].isLocalFile())
         //   ((MainWindow *)ApplicationConfig->TopLevelWindow)->FileList.append(fName);    // append file
      //}
      for(const QUrl& url : urlList)
         if (url.isLocalFile())
            ((MainWindow*)ApplicationConfig->TopLevelWindow)->FileList.append(url.toLocalFile());    // append file

   }
   event->acceptProposedAction();
   if (((MainWindow *)ApplicationConfig->TopLevelWindow)->FileList.count() > 0) 
      emit DoAddDragAndDropFile();
}
...
void cCustomSlideTable::dragMoveEvent(QDragMoveEvent *event) 
{
   ...
   int newrow = ThumbHeight > 0 ? (posY+verticalOffset())/ThumbHeight : 0;
   int newcol = ThumbWidth > 0 ? (posX+horizontalOffset())/ThumbWidth : 0;
   int Selected = newrow * NbrX + newcol;
   bool MusicPart = ((posY+verticalOffset())-newrow*ThumbHeight)>=3*(ThumbHeight/4);
   //bool BackgroundPart =((posY+verticalOffset())-newrow*ThumbHeight)<(ThumbHeight/4);
   ...
   if (IsDragOn == DRAGMODE_EXTERNALADD_SLIDE) 
   {
      if (Selected > Diaporama->slides.count()) 
         Selected = Diaporama->slides.count();
      else if ((Selected == Diaporama->slides.count()-1) && (posX > 0) && (posX < width()))
      {
         int NewX = posX - ( (PartitionMode ? posX : viewport()->width())/ThumbWidth)*ThumbWidth;
         if (NewX >= 3*ThumbWidth/4) 
         {
            Selected = Diaporama->slides.count();
         }
      }
   ...
   if (  (posX >= 0) && (posX <= viewport()->width())
      && (posY >= 0) && (posY <= viewport()->height()))
   {
      CursorPosValide = false;
      if (IsDragOn == DRAGMODE_EXTERNALADD_MUSIC) 
      {
         if ((Selected >= 0) && (Selected<Diaporama->slides.count()) 
            && (posX >= -columnWidth(0)) && (posX <= width()+columnWidth(0))
            && (posY >= 0 - (PartitionMode ? rowHeight(0) : 0)) &&(posY <= height()+(PartitionMode ? rowHeight(0):0)))
            {
               if ((MusicPart)/*&&(!Diaporama->List[Selected]->MusicType)*/) 
                  CursorPosValide = true;
         }
      } 
      else if (IsDragOn == DRAGMODE_EXTERNALADD_SLIDE) 
      {
         if ((Selected != -1) && (posX >= -columnWidth(0)) && (posX <= width()+columnWidth(0))
            && (posY >= 0-(PartitionMode ? rowHeight(0):0)) && (posY <= height()+(PartitionMode ? rowHeight(0):0)))
         {
            CursorPosValide=true;
         }
   ...
   if (Selected != DragItemDest || AncCursorPosValide != CursorPosValide) 
   {
      ...
      {
         if (Selected < Diaporama->slides.count())   
            update(model()->index(Selected/NbrX,Selected-(Selected/NbrX)*NbrX));
         else
            update(model()->index((Selected-1)/NbrX,(Selected-1)-((Selected-1)/NbrX)*NbrX));
      }
   }
   if (CursorPosValide) 
      event->acceptProposedAction(); 
...
void cCustomSlideTable::mouseMoveEvent(QMouseEvent *event) 
{
   ...
   if (!PartitionMode && (posX < 5/*0*/) && (horizontalScrollBar()->value() > 0))
      horizontalScrollBar()->setValue(horizontalScrollBar()->value() - 1);  // Try to scroll left if not partition mode
   else if (!PartitionMode && (posX > width() - 5) && (horizontalScrollBar()->value() < horizontalScrollBar()->maximum()))
      horizontalScrollBar()->setValue(horizontalScrollBar()->value() + 1);  // Try to scroll right if not partition mode
   else if (PartitionMode && (posY < 0) && (verticalScrollBar()->value() > 0))
      verticalScrollBar()->setValue(verticalScrollBar()->value() - 1);      // Try to scroll up if partition mode
   else if (PartitionMode && (posY > height()) && (verticalScrollBar()->value() < verticalScrollBar()->maximum()))
      verticalScrollBar()->setValue(verticalScrollBar()->value() + 1);      // Try to scroll down if partition mode
   ...
   if ((Selected != -1) && viewport()->rect().contains(event->pos()))
   {
      CursorPosValide = false;
      QRect positionRect(-columnWidth(0), 0 - (PartitionMode ? rowHeight(0) : 0), width() + columnWidth(0), height() + (PartitionMode ? rowHeight(0) : 0));
      if (IsDragOn == DRAGMODE_INTERNALMOVE_MUSIC)
      {
         ...
         if( positionRect.contains(event->pos()))
         {
            if ((Selected != Diaporama->CurrentCol) && (Selected < Diaporama->slides.count()) && (MusicPart)/*&&(!Diaporama->List[Selected]->MusicType)*/) 
               CursorPosValide = true;
         }
      }
      else if (IsDragOn == DRAGMODE_INTERNALMOVE_BACKGROUND) 
      {
         ...
         if (positionRect.contains(event->pos()))
         {
            if ((Selected != Diaporama->CurrentCol) && (Selected < Diaporama->slides.count()) && (BackgroundPart) && (!Diaporama->slides[Selected]->BackgroundType)) 
               CursorPosValide = true;
         }
      }
      else if (IsDragOn == DRAGMODE_INTERNALMOVE_SLIDE) 
      {
         ...
         if (positionRect.contains(event->pos()))
         {
            if ((Selected != Diaporama->CurrentCol) && (Selected != Diaporama->CurrentCol + 1)) 
               CursorPosValide = true;
         }
      }
   }
...
void cCustomSlideTable::mousePressEvent(QMouseEvent *event) 
{
   ...
   if ( columnCount() == 0 || Diaporama->slides.count() == 0 || IsDragOn == DRAGMODE_INTERNALMOVE_SLIDE || IsDragOn == DRAGMODE_INTERNALMOVE_MUSIC || IsDragOn == DRAGMODE_INTERNALMOVE_BACKGROUND) 
   {
      QTableWidget::mousePressEvent(event);
      return;
   }
   ...
   else if (event->modifiers() == Qt::ControlModifier) 
   {
      ...
      if (Selected != Diaporama->CurrentCol) 
   ...
   if ( Selected >= 0 && Selected < Diaporama->slides.count()) 
   {
      if (MusicPart && Diaporama->slides[Selected]->MusicType && Diaporama->slides[Selected]->MusicList.count() > 0) 
         IsDragOn = DRAGMODE_INTERNALMOVE_MUSIC;
      else 
         if (BackgroundPart && Diaporama->slides[Selected]->BackgroundType) 
            IsDragOn = DRAGMODE_INTERNALMOVE_BACKGROUND;
      else 
...
void cCustomSlideTable::mouseDoubleClickEvent(QMouseEvent *event) 
{
   ...
   if (Selected >=0 && Selected < Diaporama->slides.count()) 
   {
      if (MusicPart)              
         emit EditMusicTrack();
      else if (BackgroundPart)    
         emit EditBackground();
      else if (x<=TransitionSize) 
         emit EditTransition();
      else
         emit EditMediaObject();
   }
}
...
{
   //qDebug() << "WheelEvent, angleDelta " << event->angleDelta() << ", delta " << event->pixelDelta();
   if (!PartitionMode) 
   {
...
void cCustomSlideTable::AddObjectToTimeLine(int CurIndex) 
{
   ...
   int NbrY = NbrX > 0 ? Diaporama->slides.count() / NbrX : 0;

   if (NbrX*NbrY < Diaporama->slides.count())
      NbrY++;

   // Remove empty cell with unselecable item (if needed)
   for (int Row = 0; Row < rowCount(); Row++)
      for (int Col = 0; Col < columnCount(); Col++)
         if (cellWidget(Row, Col) != NULL)
   ...
   if (PartitionMode)
   {
      ...
      setColumnCount(NbrX);   
      for (int Col = 0; Col < columnCount(); Col++) 
         setColumnWidth(Col, ThumbWidth);
      setRowCount(NbrY);      
      for (int Row = 0; Row < rowCount(); Row++)    
         setRowHeight(Row, ThumbHeight);
   }
   else
   {
      setRowCount(1);                             
      setRowHeight(0, ThumbHeight);
      setColumnCount(Diaporama->slides.count());    
      for (int i = 0; i < columnCount(); i++) 
         setColumnWidth(i, ThumbWidth);
   }
...
void cCustomSlideTable::CurrentSelectionList(QList<int> *List) 
{
   ...
   for (int i = 0; i < IsSelected.count(); i++)
      if ((i < Diaporama->slides.count()) && (IsSelected[i]))
         List->append(i);
}
...
void cCustomSlideTable::CleanAll() 
{
   setUpdatesEnabled(false);
   for (int Row = 0; Row < rowCount(); Row++) 
      for (int Col = 0; Col < columnCount(); Col++) if (cellWidget(Row, Col) != NULL) 
         removeCellWidget(Row, Col);
   while (columnCount() > 0) 
