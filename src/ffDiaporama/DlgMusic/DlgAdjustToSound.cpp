...
DlgAdjustToSound::DlgAdjustToSound(QList<int> *ObjectList,cDiaporama *Diaporama,cApplicationConfig *ApplicationConfig,QWidget *parent) : 
   QCustomDialog(ApplicationConfig,parent), ui(new Ui::DlgAdjustToSound) 
{
   ui->setupUi(this);
...
void DlgAdjustToSound::DoInitDialog() 
{
   ...
   cMusicObject     *CurMusic        = Diaporama->GetMusicObject(Current,StartPosition);              // compute StartPosition
   ...
   for (int i = 0; i < ObjectList->count();i++) 
   {
      ...
      PrevObject = Num > 0 ? Diaporama->slides[Num-1] : NULL;
      CurObject = Diaporama->slides[Num];
      if (!CurObject->MusicPause) 
         SelectionDuration = SelectionDuration + CurObject->GetDuration() - (Num > 0 ? CurObject->GetTransitDuration() : 0);
    }
    PrevObject = Current > 0 ? Diaporama->slides[Current-1] : NULL;
    CurObject = Diaporama->slides[Current];
    NextObject = ObjectList->last() < Diaporama->slides.count()-1 ? Diaporama->slides[ObjectList->last()+1] : NULL;

    if ((CurMusic) && (StartPosition >= (QTime(0,0,0,0).msecsTo(CurMusic->GetDuration())-CurObject->TransitionDuration)))
      CurMusic = NULL;
    if ((PrevMusic) && (PrevObject) && (PrevStartPosition >= (QTime(0,0,0,0).msecsTo(PrevMusic->GetDuration())-PrevObject->TransitionDuration)))
      PrevMusic = NULL;
    ...
    const QString tFormat("hh:mm:ss.zzz");
    ...
    ui->Duration->setText(QTime(0,0,0,0).addMSecs(TrackDuration).toString(tFormat));

    ui->SlidesInSelection->setText(QString("%1").arg(ObjectList->count()));
    ui->TimePlayed->setText(QTime(0,0,0,0).addMSecs(SelectionDuration).toString(tFormat));

    ui->TimeBefore->setText(StartPosition > 0 ? QTime(0,0,0,0).addMSecs(StartPosition).toString(tFormat) : "-");
    ui->TimeRemaining->setText(Remaining == 0 ? "-" : Remaining > 0 ? QTime(0,0,0,0).addMSecs(Remaining  ).toString(tFormat) : "-" + QTime(0,0,0,0).addMSecs(-Remaining  ).toString(tFormat));
    ui->AdjustBefore->setText(Remaining == 0 ? "-" : Remaining  > 0 ? QTime(0,0,0,0).addMSecs(Remaining  ).toString(tFormat) : "-" + QTime(0,0,0,0).addMSecs(-Remaining  ).toString(tFormat));
    ui->AdjustWith->setText(MissingWith == 0 ? "-" : MissingWith > 0 ? QTime(0,0,0,0).addMSecs(MissingWith).toString(tFormat) : "-" + QTime(0,0,0,0).addMSecs(-MissingWith).toString(tFormat));

    if (Remaining == 0)
      ui->OKBefore->setEnabled(false);
    if (Remaining == Transition)
      ui->OKWith->setEnabled(false);
...
void DlgAdjustToSound::DoAdjust(int64_t WantedDuration) 
{
   ...
   for (i = 0; i < ObjectList->count(); i++) 
      CumuledTransition += Diaporama->slides[ObjectList->at(i)]->GetTransitDuration();

   for (i = 0 ; i < ObjectList->count(); i++) 
   {
      int Num = ObjectList->at(i);
      cDiaporamaObject *CurObject = Diaporama->slides[Num];

      if (!CurObject->MusicPause) 
      {
         int64_t CurNewDuration    = 0;
         int64_t Transition        = Num > 0 ? Diaporama->slides[Num]->GetTransitDuration() : 0;
         ...
         for (int ShotNum = 0; ShotNum < CurObject->shotList.count(); ShotNum++)
         {
            cDiaporamaShot *Shot = CurObject->shotList[ShotNum];
   ...
   if (NewSelectionDuration != 0 && Latest != NULL)
      Latest->StaticDuration -= NewSelectionDuration;
   done(0);
}
