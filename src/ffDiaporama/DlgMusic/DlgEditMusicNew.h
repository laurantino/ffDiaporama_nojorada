...
#ifndef DLGEDITMUSICNEW_H
#define DLGEDITMUSICNEW_H
...
class DlgEditMusicNew : public QCustomDialog 
{
      Q_OBJECT
   public:
      QSemaphore sem;
      volatile bool stopPrepFrames;
      ...
      // Thread controls
      QMutex     PlayerMutex;
      cFrameList FrameList;              // Collection of buffered frames
    ...
    void setAudioOutput();
      ...
      explicit DlgEditMusicNew(cMusicObject *MusicObject,cApplicationConfig *ApplicationConfig,QWidget *parent=0);
      ~DlgEditMusicNew();

      // function to be overloaded from QCustomDialog
   ...
   private slots:
      ...
      void s_PositionChangeByUser(int);
      ...
      void s_stateChanged(QAudio::State state);
      void notify();
      void soundToDevice();

   private:
      void PrepareSoundFrame(cDiaporamaObjectInfo *NewFrame,int Position);
      void PrepareSoundFrames(int startPosition);
