    ...
    if (DiaporamaObject->MusicType) {
        for (int CurIndex = 0; CurIndex<DiaporamaObject->MusicList.count(); CurIndex++) 
        {
            ...
            if (!ok) 
               ...
               qVolume = qreal(qVolume)/100;
            cMusicObject *MusicObject = DiaporamaObject->MusicList.MusicObjectAt(CurIndex);
            MusicObject->AllowCredit = GetCBChecked(CurIndex);
            MusicObject->Volume = qVolume;
            MusicObject->ForceFadIn = int64_t(FadeIN.toDouble()*1000);
            MusicObject->ForceFadOut = int64_t(FadeOUT.toDouble()*1000);
        }
    } 
    else 
      DiaporamaObject->MusicList.clear();
    DiaporamaObject->pDiaporama->clearResolvedTexts();
    return true;
}
...
{
   cMusicObject *MusicObject = DiaporamaObject->MusicList.MusicObjectAt(MusicIndex);
    QImage Icon = MusicObject->GetIcon(cCustomIcon::ICON100,false);
    if (!Icon.isNull()) Icon=Icon.scaledToHeight(32);

    QString SN = MusicObject->ShortName();
    if (SN.length()>30) 
    {
        int i=SN.length()/2;
    ...
    Item=new QTableWidgetItem(MusicObject->GetDuration().toString("hh:mm:ss.zzz")+"/"+MusicObject->GetRealDuration().toString("hh:mm:ss.zzz"));
    Item->setTextAlignment(Qt::AlignHCenter|Qt::AlignVCenter);
    if (!MusicObject->IsComputedAudioDuration) 
       Item->QT_SETTEXTOLOR(Qt::red);
    ...
    if (MusicObject->Volume==-1) InternalCB->setCurrentIndex(InternalCB->count()-1);
    else InternalCB->setCurrentIndex(InternalCB->findText(QString("%1%").arg(int(MusicObject->Volume*100))));
    ...
    InternalChB->setChecked(MusicObject->AllowCredit);
    ...
    FadeINCB->setCurrentIndex(FadeINCB->findText(QString("%1").arg(double(MusicObject->ForceFadIn)/1000,4,'f',1,'0')));
    ...
    FadeOUTCB->setCurrentIndex(FadeOUTCB->findText(QString("%1").arg(double(MusicObject->ForceFadOut)/1000,4,'f',1,'0')));
...
void DlgMusicProperties::RefreshControl(bool RefreshList) 
{
   if (DiaporamaObject != NULL)
   {
      if (RefreshList)
      {
         while (ui->PlayListTable->rowCount() > 0) 
   ...
   if ((DiaporamaObject != NULL) && (DiaporamaObject->MusicType))
   {
      ...
      int TotalDuration = 0;
      for (int CurIndex = 0; CurIndex < DiaporamaObject->MusicList.count(); CurIndex++) 
         TotalDuration += QTime(0, 0, 0, 0).msecsTo(DiaporamaObject->MusicList.MusicObjectAt(CurIndex)->GetDuration());
   ...
   if ((CurIndex < 0) || (CurIndex >= (DiaporamaObject ? DiaporamaObject->MusicList.count() : 0)))
      ui->FilePath->setText("");
   else 
      ui->FilePath->setText(QFileInfo(DiaporamaObject->MusicList.MusicObjectAt(CurIndex)->FileName()).absoluteFilePath());
}
...
void DlgMusicProperties::s_AddMusic() {
    CancelAction=false;
    FileList.clear();
    DlgFileExplorer Dlg(BROWSER_TYPE_SOUNDONLY,true,false,false,QApplication::translate("DlgMusicProperties","Add music files"),DiaporamaObject->pDiaporama->ApplicationConfig(),this);
    Dlg.InitDialog();
...
void DlgMusicProperties::s_Action_AddMusic() {
    ...
    cMusicObject *MusicObject = DiaporamaObject->MusicList.MusicObjectAt(CurIndex);
    if (MusicObject->GetInformationFromFile(NewFile,NULL,&ModifyFlag) && (MusicObject->CheckFormatValide(this))) 
    {
        if (!MusicObject->IsComputedAudioDuration) {
            ...
            if (!MusicObject->IsComputedAudioDuration)
                MusicObject->DoAnalyseSound(&Peak,&Moyenne,&CancelAction,&DlgWorkingTaskDialog->TimerProgress);
...
void DlgMusicProperties::s_EditMusic() {
    int CurIndex=ui->PlayListTable->currentRow();
    if (CurIndex==-1) return;
    DlgEditMusicNew Dlg(DiaporamaObject->MusicList.MusicObjectAt(CurIndex),ApplicationConfig,this);
    Dlg.InitDialog();
