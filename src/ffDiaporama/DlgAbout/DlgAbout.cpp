...
void DlgAbout::DoInitDialog() 
{
   ...
   //if (ApplicationConfig->CurrentLanguage != "fr") 
      FName = ApplicationConfig->StartingPath + QDir::separator() + QString("changelog-en.txt");
   //else 
   //   FName = ApplicationConfig->StartingPath + QDir::separator() + QString("changelog-fr.txt");
   ...
   if (ApplicationConfig->CurrentLanguage != "en")
   {
      ...
      if (File.open(QIODevice::ReadOnly | QIODevice::Text))
      {
         while (!File.atEnd())
         {
            ...
            if (Line.indexOf("=") < 0) 
               Line = QApplication::translate("DlgAbout", "Interface translation version: ") + Line + "\n\ten\t100%\n"; 
            else
            {
               if (Line.indexOf("to translate")) 
                  Line.replace("to translate", QApplication::translate("DlgAbout", "to translate"));
               Line = "\t" + Line;
               Line.replace(" = ", "\t");
            }
            Text.append(Line);
   ...
   if (!Text.isEmpty()) 
      ui->TranslationED->setText(Text); else ui->tabWidget->removeTab(4);
