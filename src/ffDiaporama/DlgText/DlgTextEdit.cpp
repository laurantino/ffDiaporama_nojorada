...
void DlgTextEdit::DoInitDialog() 
{
    ...
    for (int i = 0; i < sizes.count(); i++)
       Ssizes.append(QString("%1").arg(sizes[i]));
    ...
    ui->TextEdit->setHtml(CurrentTextItem->tParams.Text);
    ...
    ui->fontEffectCB->addItem(QIcon(":/img/Text/Shadow_upper_left.png"),    QApplication::translate("DlgTextEdit","Shadow upper left"));
    ui->fontEffectCB->addItem(QIcon(":/img/Text/Shadow_upper_right.png"),   QApplication::translate("DlgTextEdit","Shadow upper right"));
    ui->fontEffectCB->addItem(QIcon(":/img/Text/Shadow_bottom_left.png"),   QApplication::translate("DlgTextEdit","Shadow bottom left"));
    ui->fontEffectCB->addItem(QIcon(":/img/Text/Shadow_bottom_right.png"),  QApplication::translate("DlgTextEdit","Shadow bottom right"));
    ...
    connect(ui->TextEdit, SIGNAL(sizeChanged()), this, SLOT(onTextEditSizeChanged()));
    RefreshControls();
}
...
void DlgTextEdit::s_cursorPositionChanged() 
{
    ...
    if (Cursor.hasSelection())  
       TCF=Cursor.charFormat();                // Display option for current selection
    else
       TCF=ui->TextEdit->currentCharFormat();  // Display current default option

    Alignment=ui->TextEdit->alignment();
    CurrentTextItem->tParams.FontColor = TCF.foreground().color().name(QColor::HexArgb);
    //qDebug() << "colorName " << TCF.foreground().color().name(QColor::HexArgb);
    ...
    int indent=0;
    if (List) 
       indent=List->format().indent(); else indent=TBF.indent();
...
void DlgTextEdit::RefreshControls() 
{
   ...
   // Margins
   ui->ShapeDefaultBT->setChecked( CurrentTextItem->tParams.TMType == eSHAPEDEFAULT);
   ui->FullShapeBT->setChecked(    CurrentTextItem->tParams.TMType == eFULLSHAPE);
   ui->CustomBT->setChecked(       CurrentTextItem->tParams.TMType == eCUSTOM);
   ui->PosSize_X->setEnabled(      CurrentTextItem->tParams.TMType == eCUSTOM);
   ui->PosXEd->setEnabled(         CurrentTextItem->tParams.TMType == eCUSTOM);
   ui->PosSize_Y->setEnabled(      CurrentTextItem->tParams.TMType == eCUSTOM);
   ui->PosYEd->setEnabled(         CurrentTextItem->tParams.TMType == eCUSTOM);
   ui->PosSize_Width->setEnabled(  CurrentTextItem->tParams.TMType == eCUSTOM);
   ui->WidthEd->setEnabled(        CurrentTextItem->tParams.TMType == eCUSTOM);
   ui->PosSize_Height->setEnabled( CurrentTextItem->tParams.TMType == eCUSTOM);
   ui->HeightEd->setEnabled(       CurrentTextItem->tParams.TMType == eCUSTOM);
   ui->PosXEd->setValue(           floor(CurrentTextItem->tParams.textMargins.x()*100));
   ui->PosYEd->setValue(           floor(CurrentTextItem->tParams.textMargins.y()*100));
   ui->WidthEd->setValue(          floor(CurrentTextItem->tParams.textMargins.width()*100));
   ui->HeightEd->setValue(         floor(CurrentTextItem->tParams.textMargins.height()*100));

   ui->textUp->setChecked(CurrentTextItem->tParams.VAlign==0);                 ui->textUp->setDown(CurrentTextItem->tParams.VAlign==0);
   ...
   // Brush TAB part
   bool Allow_Brush  =((CurrentTextItem->BackgroundBrush->BrushType!=BRUSHTYPE_IMAGEDISK)&&(CurrentTextItem->tParams.TextClipArtName==""));
   ...
   else if (CurrentTextItem->tParams.TextClipArtName == "") 
   {
      Brush = CurrentTextItem->BackgroundBrush->GetBrush(QSizeF(ui->TextEdit->width(),ui->TextEdit->height()),true,0,NULL,1,NULL);
   } 
   else 
   {
      Brush = new QBrush(TextFrameList.List[TextFrameList.SearchImage(CurrentTextItem->tParams.TextClipArtName)].BckColor,Qt::SolidPattern);
   }
   QPalette Palette;
...
void DlgTextEdit::s_SetBold() 
{
    ...
    if (Cursor.hasSelection())
       CurrentFormat = Cursor.charFormat();                // Display option for current selection
    else
       CurrentFormat = ui->TextEdit->currentCharFormat();  // Display current default option
    ...
    CurrentTextItem->tParams.IsBold = (TCF.fontWeight() == QFont::Bold);
    CurrentTextItem->tParams.Text = ui->TextEdit->toHtml();
    CurrentTextItem->resolvedText.clear();
    RefreshControls();
...
void DlgTextEdit::s_SetItalic() 
{
    ...
    CurrentTextItem->tParams.IsItalic = TCF.fontItalic();
    CurrentTextItem->tParams.Text = ui->TextEdit->toHtml();
    CurrentTextItem->resolvedText.clear();
    RefreshControls();
    ui->TextEdit->setFocus();
...
void DlgTextEdit::s_SetUnderline() 
{
    ...
    CurrentTextItem->tParams.IsUnderline = TCF.fontUnderline();
    CurrentTextItem->tParams.Text = ui->TextEdit->toHtml();
    RefreshControls();
    ui->TextEdit->setFocus();
}
...
void DlgTextEdit::s_SetTextSuper() 
{
   ...
   if (Cursor.hasSelection())  
      TCF = Cursor.charFormat();                // Modify current selection
   else
      TCF = ui->TextEdit->currentCharFormat();  // Modify default option

   if (TCF.verticalAlignment() == QTextCharFormat::AlignSuperScript) 
   { 
      TCF = QTextCharFormat();  
   ...
   { 
      TCF = QTextCharFormat();  
      TCF.setVerticalAlignment(QTextCharFormat::AlignSuperScript); 
   }
...
void DlgTextEdit::s_SetTextLeft() {
    ...
    CurrentTextItem->tParams.HAlign = 0;
    CurrentTextItem->tParams.Text = ui->TextEdit->toHtml();
    RefreshControls();
    ui->TextEdit->setFocus();
}
...
void DlgTextEdit::s_SetTextCenter() {
    ...
    CurrentTextItem->tParams.HAlign = 1;
    CurrentTextItem->tParams.Text = ui->TextEdit->toHtml();
    RefreshControls();
    ui->TextEdit->setFocus();
}
...
void DlgTextEdit::s_SetTextRight() {
    ...
    CurrentTextItem->tParams.HAlign = 2;
    CurrentTextItem->tParams.Text = ui->TextEdit->toHtml();
    RefreshControls();
    ui->TextEdit->setFocus();
}
...
void DlgTextEdit::s_SetTextJustif() {
    ...
    CurrentTextItem->tParams.HAlign = 3;
    CurrentTextItem->tParams.Text = ui->TextEdit->toHtml();
    CurrentTextItem->resolvedText.clear();
    RefreshControls();
    ui->TextEdit->setFocus();
...
void DlgTextEdit::s_SetTextUp() {
    ...
    CurrentTextItem->tParams.VAlign = 0;
...
void DlgTextEdit::s_SetTextVCenter() {
    ...
    CurrentTextItem->tParams.VAlign = 1;
...
void DlgTextEdit::s_SetTextBottom() {
    ...
    CurrentTextItem->tParams.VAlign = 2;
...
void logCharFormat(const QTextCharFormat TCF, QString what)
{
   qDebug() << what;
   //qDebug() << "font families " << TCF.fontFamilies();
   qDebug() << "font familie "  << TCF.fontFamily();
   qDebug() << "font " <<TCF.font();
   qDebug() << " ";
}
//========= Font family
void DlgTextEdit::s_ChangeFont(QFont font) 
{
   ...
   TCF.setFontFamilies(QStringList(font.family()));
   #endif
   //logCharFormat(TCF, "to set");
   //TCF.setFont(font);
   if (Cursor.hasSelection())
   {
      //TCF = Cursor.charFormat();
      //TCF.setFontFamily(font.family());
      //logCharFormat(Cursor.charFormat(), "before merge");
      Cursor.mergeCharFormat(TCF);
   ...
   //logCharFormat(Cursor.charFormat(), "after merge");
   CurrentTextItem->tParams.FontName = font.family();
   CurrentTextItem->tParams.Text = ui->TextEdit->toHtml();
   CurrentTextItem->resolvedText.clear();
   RefreshControls();
   ui->TextEdit->setFocus();
...
void DlgTextEdit::s_ChangeSizeFont(QString size) 
{
   ...
   CurrentTextItem->tParams.FontSize = size.toInt();
   CurrentTextItem->tParams.Text = ui->TextEdit->toHtml();
   CurrentTextItem->resolvedText.clear();
   RefreshControls();
   ui->TextEdit->setFocus();
...
void DlgTextEdit::s_ChangeStyleFont(int Style) {
    ...
    CurrentTextItem->tParams.StyleText = Style;
    RefreshControls();
}
...
void DlgTextEdit::s_TextEditChange() {
    ...
    if (CurrentTextItem->tParams.Text != TextEditText && CurrentPlainText != PlainTextEdit) 
    {
...
void DlgTextEdit::s_ChIndexFontColorCombo(int) 
{
   ...
   TCF.setForeground(QBrush(QColor(CurrentTextItem->tParams.FontColor)));
//   qDebug() << " set Foreground Color to " << CurrentTextItem->tParams.FontColor;
//   qDebug() << "colorName now" << TCF.foreground().color().name(QColor::HexArgb);
   if (Cursor.hasSelection())  
      Cursor.mergeCharFormat(TCF);
   else
      ui->TextEdit->mergeCurrentCharFormat(TCF);
...
void DlgTextEdit::s_ChIndexGradientFirstColorCombo(int index) 
{
   Q_UNUSED(index)
...
void DlgTextEdit::s_ShapeDefault_Changed() {
    ...
    CurrentTextItem->ApplyTextMargin(eSHAPEDEFAULT);
    RefreshControls();
}
...
void DlgTextEdit::s_FullShape_Changed() {
    ...
    CurrentTextItem->ApplyTextMargin(eFULLSHAPE);
    RefreshControls();
}
...
void DlgTextEdit::s_Custom_Changed() {
    ...
    CurrentTextItem->ApplyTextMargin(eCUSTOM);
    RefreshControls();
}
...
void DlgTextEdit::s_PosXEd_Changed(int) 
{
   ...
   CurrentTextItem->tParams.textMargins.moveLeft(double(ui->PosXEd->value())/100.0);
   if (CurrentTextItem->tParams.textMargins.x() + CurrentTextItem->tParams.textMargins.width() > 1) 
      CurrentTextItem->tParams.textMargins.moveLeft(1-CurrentTextItem->tParams.textMargins.width());
   RefreshControls();
}
...
void DlgTextEdit::s_PosYEd_Changed(int) 
{
   ...
   AppendPartialUndo(UNDOSTYLE_MARGINS,ui->CustomBT,false);
   CurrentTextItem->tParams.textMargins.moveTop(double(ui->PosYEd->value())/100.0);
   if (CurrentTextItem->tParams.textMargins.y() + CurrentTextItem->tParams.textMargins.height() > 1) 
      CurrentTextItem->tParams.textMargins.moveTop(1-CurrentTextItem->tParams.textMargins.height());
   //if (CurrentTextItem->tParams.TMy + CurrentTextItem->tParams.TMh > 1) 
   //   CurrentTextItem->tParams.TMy = 1-CurrentTextItem->tParams.TMh;
   RefreshControls();
}
...
void DlgTextEdit::s_WidthEd_Changed(int) 
{
   ...
   AppendPartialUndo(UNDOSTYLE_MARGINS,ui->CustomBT,false);
   CurrentTextItem->tParams.textMargins.setWidth(double(ui->WidthEd->value())/100.0);
   if (CurrentTextItem->tParams.textMargins.x() + CurrentTextItem->tParams.textMargins.width() > 1) 
      CurrentTextItem->tParams.textMargins.setWidth(1-CurrentTextItem->tParams.textMargins.x());
   //if (CurrentTextItem->tParams.TMx + CurrentTextItem->tParams.TMw > 1) 
   //   CurrentTextItem->tParams.TMw = 1-CurrentTextItem->tParams.TMx;
   RefreshControls();
}
...
void DlgTextEdit::s_HeightEd_Changed(int) 
{
   ...
   AppendPartialUndo(UNDOSTYLE_MARGINS,ui->CustomBT,false);
   CurrentTextItem->tParams.textMargins.setHeight(double(ui->HeightEd->value())/100.0);
   if (CurrentTextItem->tParams.textMargins.y() + CurrentTextItem->tParams.textMargins.height() > 1) 
      CurrentTextItem->tParams.textMargins.setHeight(1-CurrentTextItem->tParams.textMargins.y());
   //if (CurrentTextItem->tParams.TMy + CurrentTextItem->tParams.TMh > 1) 
   //   CurrentTextItem->tParams.TMh = 1-CurrentTextItem->tParams.TMy;
   RefreshControls();
}

void DlgTextEdit::resizeEvent(QResizeEvent *event)
{
   QCustomDialog::resizeEvent(event);
   RefreshControls();
}
