...
void wgt_QGMapsMap::DoInitDialog() {
    ...
    if ((!CurrentMap->RequestList.isEmpty())||((CurrentMap->locations.count()>0)&&(!CurrentMap->IsMapValide))) QTimer::singleShot(LATENCY,this,SLOT(RestartRequest()));
}
...
void wgt_QGMapsMap::RefreshControls() {
    ...
    ui->LocationTable->setRowCount(CurrentMap->locations.count());
    ui->LocationTable->setUpdatesEnabled(true);

    // Fill Map Size CB
    UpdateMapSizes();

    cLocation                   *CurLocation=(cLocation *)(GetCurLocationIndex()!=-1?CurrentMap->locations[GetCurLocationIndex()]:NULL);
    ...
    ui->MapSizeCB->setEnabled(!CurrentMap->locations.isEmpty());
    ui->EditLocationBT->setEnabled(CurLocation);
    ui->RemoveLocationBT->setEnabled(CurrentMap->locations.count()>0);
    ExportMapBT->setEnabled(CurrentMap->RequestList.isEmpty() && !CurrentMap->locations.isEmpty());
    StopMaj=false;
}
...
void wgt_QGMapsMap::ResetDisplayMap() {
    // Compute marker size
    for (int i=0;i<CurrentMap->locations.count();i++) CurrentBrush->ComputeMarkerSize(CurrentMap->locations[i],CurrentMap->GetCurrentImageSize());
    // Setup map
...
void wgt_QGMapsMap::UpdateMapSizes() {
    if (CurrentMap->IsMapValide) {
        ui->MapSizeCB->clear();
        if (CurrentMap->locations.count()>0) {
...
void wgt_QGMapsMap::RequestGoogle(bool DuplicateRessource) {
    ...
    DlgGMapsGeneration Dlg(CurrentBrush,CurrentMap,DuplicateRessource,pAppConfig,this);
    Dlg.InitDialog();
    Dlg.exec();
    pAppConfig->ImagesCache.RemoveImageObject(PrevRessourceKey,-1);
    ResetDisplayMap();
    emit DoRefreshImageObject();
}
...
void wgt_QGMapsMap::AddGMapsLocation() {
    ...
    if (Action) {
        ...
        if (ActionType==FAVACTIONTYPE_ADDPROJECT) {
            Location=new cLocation(pAppConfig);
            ...
            Location=new cLocation(pAppConfig);
            DlgGMapsLocation Dlg(Location,pAppConfig,this);
            Dlg.InitDialog();
            if (Dlg.exec()!=0) {
                delete Location;
                Location=NULL;
    ...
    if (Location) {
        ...
        ui->LocationTable->setUpdatesEnabled(false);
        CurrentMap->locations.append(Location);
        ...
        int     IndexKey = (Object ? ((cCompositionObject *)Object)->index() : -1);
        bool    Found   = false;
        if (IndexKey!=-1) 
        {
            ...
            if (DiaporamaObject) 
            {
               ...
               foreach(cDiaporamaShot *shot,DiaporamaObject->shotList)
               {
                  cCompositionObject *obj = shot->ShotComposition.getCompositionObject(IndexKey);
                  if( obj )
                  {
                     Found = true;
                     obj->BackgroundBrush->Markers.append(Marker);
                  }
               }
            }
        }
...
void wgt_QGMapsMap::RemoveLocation() {
    ...
    if (CurIndex!=-1) {
        ...
        ui->LocationTable->setUpdatesEnabled(false);
        ui->LocationTable->setRowCount(CurrentMap->locations.count()-1);
        delete (cLocation *)CurrentMap->locations.takeAt(CurIndex);
        ...
        if (IndexKey != -1) 
        {
            //while ((Object)&&(Object->objectName()!="cDiaporamaObject")) 
            //   Object=Object->parent();
            cDiaporamaObject *DiaporamaObject = (cDiaporamaObject *)CurrentBrush->GetDiaporamaObject();//(cDiaporamaObject *)Object;
            if (DiaporamaObject) for (int i = 0; i<DiaporamaObject->shotList.count(); i++) for (int j = 0; j<DiaporamaObject->shotList[i]->ShotComposition.compositionObjects.count(); j++)
            {
               if (DiaporamaObject->shotList[i]->ShotComposition.compositionObjects[j]->index() == IndexKey)
               {
                    Found=true;
                    DiaporamaObject->shotList[i]->ShotComposition.compositionObjects[j]->BackgroundBrush->Markers.removeAt(CurIndex);
                }
            }
        }
        ...
        ui->LocationTable->setUpdatesEnabled(true);
        if ((CurrentMap->locations.count() > 0) && (CurrentMap->IsMapValide)) 
         ResetDisplayMap(); 
        else 
         UpdateDisplayMap();
    }
...
int wgt_QGMapsMap::GetCurLocationIndex() {
    ...
    if ((CurIndex<0)||(CurIndex>=CurrentMap->locations.count())) return -1;
    return CurIndex;
}
...
void wgt_QGMapsMap::DoubleClickedLocation(QModelIndex) {
    ...
    int CurIndex=GetCurLocationIndex();
    if (CurIndex!=-1) {
        cLocation *CurLocation=((cLocation *)CurrentMap->locations.at(CurIndex));
        if (CurLocation->LocationType==cLocation::FREE) {
            double GPSxOld=CurLocation->GPS_cx;
            double GPSyOld=CurLocation->GPS_cy;
            ParentDialog->AppendPartialUndo(DlgImageCorrection::UNDOACTION_GMAPSMAPPART,ui->LocationTable,true,this,true);
            DlgGMapsLocation Dlg(CurLocation,pAppConfig,this);
            Dlg.InitDialog();
            if (Dlg.exec()==0) {
                ui->LocationTable->setUpdatesEnabled(false);
                ui->LocationTable->setUpdatesEnabled(true);
...
void wgt_QGMapsMap::PointFormChanged(int newform) {
    if (StopMaj) return;
    int Index=GetCurLocationIndex();    if ((Index<0)||(Index>=CurrentMap->locations.count())) return;
    ParentDialog->AppendPartialUndo(DlgImageCorrection::UNDOACTION_GMAPSMAPPART,ui->PointFormCB,true,this,true);
    ((cLocation *)CurrentMap->locations[Index])->MarkerPointForm=(cLocation::MARKERPOINT)newform;
    ResetDisplayMap();
}
...
void wgt_QGMapsMap::MarkerFormChanged(int newform) {
    ...
    ((cLocation *)CurrentMap->locations[Index])->MarkerForm=(cLocation::MARKERFORM)newform;
    ResetDisplayMap();
}
...
void wgt_QGMapsMap::MarkerSizeChanged(int newsize) {
    ...
    ((cLocation *)CurrentMap->locations[Index])->Size=(cBrushDefinition::sMarker::MARKERSIZE)newsize;
    ResetDisplayMap();
}
...
void wgt_QGMapsMap::MarkerCompoChanged(int newcompo) {
    ...
    ((cLocation *)CurrentMap->locations[Index])->MarkerCompo=(cLocation::MARKERCOMPO)newcompo;
    ResetDisplayMap();
}
...
void wgt_QGMapsMap::DistanceChanged(int newdistance) {
    ...
    ((cLocation *)CurrentMap->locations[Index])->Distance=(cLocation::MARKERDISTANCE)newdistance;
    ResetDisplayMap();
}
...
void wgt_QGMapsMap::ExportMap() {
    QString OutputFileName=QDir::toNativeSeparators(pAppConfig->SettingsTable->GetTextValue(QString("%1_path").arg(BrowserTypeDef[BROWSER_TYPE_CAPTUREIMAGE].BROWSERString),DefaultProjectPath));
    ...
    if (OutputFileName!="") {
        if (pAppConfig->RememberLastDirectories) pAppConfig->SettingsTable->SetTextValue(QString("%1_path").arg(BrowserTypeDef[BROWSER_TYPE_CAPTUREIMAGE].BROWSERString),QFileInfo(OutputFileName).absolutePath());     // Keep folder for next use
