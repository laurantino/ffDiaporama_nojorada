...
void wgt_QEditVideo::DoInitWidget(QCustomDialog *ParentDialog,cBrushDefinition *CurrentBrush) 
{
    ...
    WantedFPS               = pAppConfig->PreviewFPS;
    IsValide                = true;
    ...
    // to check
    if (FileInfo->EndTime >= FileInfo->GetRealDuration()) 
      FileInfo->EndTime = FileInfo->GetRealDuration().addMSecs(-1);

    // Init embeded widgets
    for (int Factor = 150; Factor >= 0; Factor -= 10)
       ui->VolumeReductionFactorCB->addItem(QString("%1%").arg(Factor));
...
void wgt_QEditVideo::setAudioOutput()
{
   // Set up the format
   QAudioFormat format;
...
void wgt_QEditVideo::DoInitDialog() {
    ...
    SetCurrentPos(FileInfo->StartTime);
    RefreshControls();

    if (FileInfo->LibavStartTime > 0) 
      ui->VideoStartLabel->setText(QApplication::translate("wgt_QEditVideo","Video stream start position is %1").arg(QTime(0,0,0,0).addMSecs(FileInfo->LibavStartTime/1000).toString("hh:mm:ss.zzz")));
   else 
...
void wgt_QEditVideo::showEvent(QShowEvent*)
{
   if (!IsInit)
   {
      ...
      ThreadAnalyseMusic.setFuture(QT_CONCURRENT_RUN(this, &wgt_QEditVideo::s_AnalyseSound));
...
void wgt_QEditVideo::RefreshControls() 
{
    ...
    ui->CustomRuler->StartPos     = QTime(0,0,0,0).msecsTo(FileInfo->StartTime);
    ui->CustomRuler->EndPos       = QTime(0,0,0,0).msecsTo(FileInfo->EndTime);
    ui->CustomRuler->TotalDuration= QTime(0,0,0,0).msecsTo(FileInfo->GetRealDuration());
    //QTime ti = FileInfo->GetRealDuration().addMSecs(0);
    //qDebug() << ti;
    ...
    tDuration = FileInfo->GetRealDuration().addMSecs(0);
    QTime Duration = QTime(0,0,0,0).addMSecs(FileInfo->StartTime.msecsTo(FileInfo->EndTime));
    ui->DeinterlaceBt->setChecked(CurrentBrush->Deinterlace);
    ui->ActualDuration->setText(Duration.toString("hh:mm:ss.zzz"));
    ui->StartPosEd->setMaximumTime(FileInfo->EndTime);    ui->StartPosEd->setTime(FileInfo->StartTime);
    ui->EndPosEd->setMinimumTime(FileInfo->StartTime);    ui->EndPosEd->setTime(FileInfo->EndTime);
    if (CurrentBrush->SoundVolume == -1) 
      ui->VolumeReductionFactorCB->setCurrentIndex(ui->VolumeReductionFactorCB->count()-1);
    else 
    ...
    QTime t(0, 0, 0, 0);
    SetStartEndPos(t.msecsTo(FileInfo->StartTime), t.msecsTo(FileInfo->EndTime)-t.msecsTo(FileInfo->StartTime));
...
{
   SetPlayerToPause();
   QString OutputFileName = pAppConfig->SettingsTable->GetTextValue(QString("%1_path").arg(BrowserTypeDef[BROWSER_TYPE_CAPTUREIMAGE].BROWSERString), DefaultCaptureImage);
   ...
   if (OutputFileName != "")
   {
      if (pAppConfig->RememberLastDirectories)
         pAppConfig->SettingsTable->SetTextValue(QString("%1_path").arg(BrowserTypeDef[BROWSER_TYPE_CAPTUREIMAGE].BROWSERString), QFileInfo(OutputFileName).absolutePath());     // Keep folder for next use
...
void wgt_QEditVideo::s_MusicReduceFactorChange(int) 
{
   ...
   if (Volume != "")
      Volume = Volume.left(Volume.length() - 1);  // Remove %
   bool ok = true;
   double dVolume = Volume.toInt(&ok);
   if (!ok)
      dVolume = -1;
   else
      dVolume = double(Volume.toInt()) / 100;
...
void wgt_QEditVideo::s_StartEndChangeByUser() 
{
    ...
    FileInfo->StartTime = StartPos;
    FileInfo->EndTime = EndPos;
    ui->StartPosEd->setTime(FileInfo->StartTime);
    ui->EndPosEd->setTime(FileInfo->EndTime);
}
...
void wgt_QEditVideo::s_DefStartPos() {
    ...
    FileInfo->StartTime = GetCurrentPos();
    ui->CustomRuler->StartPos=QTime(0,0,0,0).msecsTo(FileInfo->StartTime);
    RefreshControls();
...
void wgt_QEditVideo::s_DefEndPos() {
    ...
    FileInfo->EndTime = GetCurrentPos();
    if (FileInfo->EndTime >= FileInfo->GetRealDuration()) 
      FileInfo->EndTime = FileInfo->GetRealDuration().addMSecs(-1);
    ui->CustomRuler->EndPos = QTime(0,0,0,0).msecsTo(FileInfo->EndTime);
    RefreshControls();
...
void wgt_QEditVideo::s_EditEndPos(QTime NewValue) {
    ...
    FileInfo->EndTime = NewValue;
    ui->EndPosEd->setTime(FileInfo->EndTime);
    SetCurrentPos(FileInfo->EndTime);
    RefreshControls();
}
...
{
   SeekPlayer(QTime(0,0,0,0).msecsTo(FileInfo->StartTime));
   SetPlayerToPause();
}
...
void wgt_QEditVideo::s_SeekRight() {
    SeekPlayer(QTime(0,0,0,0).msecsTo(FileInfo->EndTime));
    SetPlayerToPause();
}
...
{
   if ((!force) && (ThreadAnalyseMusic.isRunning())) 
      return;
...
void wgt_QEditVideo::s_VideoPlayerPlayPauseBT() 
{
   if ((!PlayerPlayMode)||((PlayerPlayMode && PlayerPauseMode)))    
      SetPlayerToPlay(false); // Stop/Pause -> play
...
void wgt_QEditVideo::s_TimerEvent() 
{
    ...
    if (FrameList.count() == 0) 
    {
        ...
        PrepareVideoFrame(NewFrame, NewFrame->Current.Object_InObjectTime);
    ...
    LastPosition=PreviousFrame->Current.Object_InObjectTime;
    NextPosition=LastPosition+int(double(1000)/WantedFPS);

    // Add image to the list if it's not full
    if (FrameList.count() < BUFFERING_NBR_FRAME && !ThreadPrepareVideo.isRunning()) 
    {
        cDiaporamaObjectInfo *NewFrame = new cDiaporamaObjectInfo(PreviousFrame,NextPosition,NULL,int(double(1000)/WantedFPS),true);
        NewFrame->Current.Object_StartTime = 0;
        ThreadPrepareVideo.setFuture(QT_CONCURRENT_RUN_2(this,&wgt_QEditVideo::PrepareVideoFrame,NewFrame,NewFrame->Current.Object_InObjectTime));
    }
    ...
    if ((m_audioOutput->state()==QAudio::ActiveState)||(m_audioOutput->state()==QAudio::IdleState)) {
        ...
        if (len>0) {
            ...
            if ((len)&&(AudioBufSize==0)&&(AudioPlayed==0)) {
                // Remove 1 frame
                delete FrameList.DetachFirstFrame();
                Mutex.unlock();
            } else {
...
void wgt_QEditVideo::PrepareVideoFrame(cDiaporamaObjectInfo *NewFrame,int Position) 
{
   ...
   if (!NewFrame->RenderedImage.isNull()) 
