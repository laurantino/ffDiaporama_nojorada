    ...
    QCustomDialog(ApplicationConfig,parent),ui(new Ui::DlgAutoTitleSlide) 
{
   ...
   ui->ModelTable->CurrentSlide = new cDiaporamaObject(DiaporamaObject->getDiaporama());
   ...
   if (IsCreation) 
   {
      ...
      CurrentSlide->getDiaporama()->UpdateChapterInformation();
   }
...
{
   cDiaporama *pDiaporama = CurrentSlide->getDiaporama();
   if (IsCreation) 
   {
   ...
   {
      setWindowTitle( QApplication::translate("DlgAutoTitleSlide","Edit a predefined title slide") + " - " + QApplication::translate("DlgSlideProperties","Slide")
         + QString(" %1/%2").arg(pDiaporama->CurrentCol+1).arg(pDiaporama->slides.count()));
      ui->OKPreviousBT->setEnabled(pDiaporama->CurrentCol>0);
      ui->OKNextBT->setEnabled(pDiaporama->CurrentCol < pDiaporama->slides.count()-1);
   }
   ...
   ui->ChapterEventDateED->setDate(CurrentSlide->OverrideProjectEventDate ? CurrentSlide->ChapterEventDate : pDiaporama->ProjectInfo()->EventDate());
   ui->ChapterDateED->setPlainText(CurrentSlide->OverrideProjectEventDate ? CurrentSlide->OverrideChapterLongDate ? CurrentSlide->ChapterLongDate:FormatLongDate(CurrentSlide->ChapterEventDate) : pDiaporama->ProjectInfo()->LongDate());
...
void DlgAutoTitleSlide::PrepareGlobalUndo() {
    ...
    CurrentSlide->SaveToXML(root,"UNDO-DLG-OBJECT",CurrentSlide->getDiaporama()->ProjectFileName,true,NULL,NULL,false);     // Save object
...
bool DlgAutoTitleSlide::DoAccept() 
{
   ...
   if (!CurrentModel.isEmpty()) 
   {
      ...
      if (CurrentSlide->OverrideProjectEventDate) 
         CurrentSlide->ChapterEventDate = ui->ChapterEventDateED->date();
      if (CurrentSlide->OverrideChapterLongDate)  
         CurrentSlide->ChapterLongDate = ui->ChapterDateED->toPlainText();
      CurrentSlide->getDiaporama()->UpdateChapterInformation();
      CurrentSlide->getDiaporama()->UpdateGMapsObject();
      emit SetModifyFlag();
      return true;
   }
   else {
...
void DlgAutoTitleSlide::OverrideProjectDateChanged(int) {
    CurrentSlide->OverrideProjectEventDate=ui->OverrideProjectDateCB->isChecked();
    if (!CurrentSlide->OverrideProjectEventDate) {
        ui->ChapterEventDateED->setDate(CurrentSlide->getDiaporama()->ProjectInfo()->EventDate());
        CurrentSlide->ChapterLongDate = FormatLongDate(CurrentSlide->getDiaporama()->ProjectInfo()->EventDate());
...
void DlgAutoTitleSlide::ChapterEventDateChanged(const QDate &NewDate) 
{
   ...
   if (!CurrentSlide->OverrideChapterLongDate) 
   {
      CurrentSlide->ChapterLongDate = CurrentSlide->OverrideProjectEventDate ? FormatLongDate(CurrentSlide->ChapterEventDate) : CurrentSlide->getDiaporama()->ProjectInfo()->LongDate();
      ui->ChapterDateED->setPlainText(CurrentSlide->ChapterLongDate);
   }
   CurrentSlide->getDiaporama()->UpdateChapterInformation();
   emit SetModifyFlag();
...
void DlgAutoTitleSlide::s_ChSlideTypeCB(int CurrentType) {
    ...
    switch (CurrentType) {
        case 0:
            for (SubType=0;SubType<MODELTYPE_PROJECTTITLE_CATNUMBER;SubType++) ui->SlideCatCB->addItem(ApplicationConfig->PrjTitleModels[CurrentSlide->getDiaporama()->ImageGeometry][SubType==9?MODELTYPE_PROJECTTITLE_CATNUMBER-1:SubType]->NameCategorie);
            CurrentSlide->StartNewChapter=false;
            break;
        case 1:
            for (SubType=0;SubType<MODELTYPE_CHAPTERTITLE_CATNUMBER;SubType++) ui->SlideCatCB->addItem(ApplicationConfig->CptTitleModels[CurrentSlide->getDiaporama()->ImageGeometry][SubType==9?MODELTYPE_CHAPTERTITLE_CATNUMBER-1:SubType]->NameCategorie);
            CurrentSlide->StartNewChapter=true;
            break;
        case 2:
            for (SubType=0;SubType<MODELTYPE_CREDITTITLE_CATNUMBER;SubType++) ui->SlideCatCB->addItem(ApplicationConfig->CreditTitleModels[CurrentSlide->getDiaporama()->ImageGeometry][SubType==9?MODELTYPE_CREDITTITLE_CATNUMBER-1:SubType]->NameCategorie);
            CurrentSlide->StartNewChapter=false;
            break;
    }
    CurrentSlide->getDiaporama()->UpdateInformation();
    ...
    switch (CurrentType) {
        case 0 : ui->ModelTable->PrepareTable(ApplicationConfig->PrjTitleModels[CurrentSlide->getDiaporama()->ImageGeometry][0]);       break;
        case 1 : ui->ModelTable->PrepareTable(ApplicationConfig->CptTitleModels[CurrentSlide->getDiaporama()->ImageGeometry][0]);       break;
        case 2 : ui->ModelTable->PrepareTable(ApplicationConfig->CreditTitleModels[CurrentSlide->getDiaporama()->ImageGeometry][0]);    break;
    }
    ui->ModelTable->SetCurrentModel(OldName.mid(QString("<%AUTOTS_").length(),QString("000000").length()));
...
void DlgAutoTitleSlide::s_ChSlideCatCB(int CurrentSubType) {
    ...
    Timer.stop();
    switch (CurrentType) {
        case 0 : ui->ModelTable->PrepareTable(ApplicationConfig->PrjTitleModels[CurrentSlide->getDiaporama()->ImageGeometry][CurrentSubType==9?MODELTYPE_PROJECTTITLE_CATNUMBER-1:CurrentSubType]);     break;
        case 1 : ui->ModelTable->PrepareTable(ApplicationConfig->CptTitleModels[CurrentSlide->getDiaporama()->ImageGeometry][CurrentSubType==9?MODELTYPE_CHAPTERTITLE_CATNUMBER-1:CurrentSubType]);     break;
        case 2 : ui->ModelTable->PrepareTable(ApplicationConfig->CreditTitleModels[CurrentSlide->getDiaporama()->ImageGeometry][CurrentSubType==9?MODELTYPE_CREDITTITLE_CATNUMBER-1:CurrentSubType]);   break;
    }
    ui->ModelTable->SetCurrentModel(OldName.mid(QString("<%AUTOTS_").length(),QString("000000").length()));
    ui->ModelTable->TimerPosition=0;
    Timer.start(TIMERFREQ);
...
void DlgAutoTitleSlide::ProjectProperties() {
    Timer.stop();
    DlgffDPjrProperties Dlg(false,CurrentSlide->getDiaporama(),ApplicationConfig,this);
