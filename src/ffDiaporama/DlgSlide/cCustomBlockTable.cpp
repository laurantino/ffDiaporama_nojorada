...
void cBlockTableItemDelegate::paint(QPainter *Painter, const QStyleOptionViewItem &option, const QModelIndex &index) const
{
   ...
   if ((ParentTable->CompositionList == NULL) || (index.row() >= ParentTable->rowCount()) || (index.column() >= ParentTable->columnCount())
      || (index.row() >= ParentTable->CompositionList->compositionObjects.count()))
      return;
   QApplication::setOverrideCursor(QCursor(Qt::WaitCursor));

   // Display adjustement
   int FontFactor = ((pAppConfig->TimelineHeight - TIMELINEMINHEIGHT) / 20 + 1) * 10;
   int RowHeight = 48 + ((pAppConfig->TimelineHeight - TIMELINEMINHEIGHT) / 20 + 1) * 3;

   cCompositionObject *compositionObject = ParentTable->CompositionList->compositionObjects[index.row()];
   if (!compositionObject->isVisible())
      Painter->setOpacity(0.5);

   cBaseMediaFile *MediaFile = compositionObject->BackgroundBrush->MediaObject;
   ...
   if (MediaFile != NULL)
   {
      ...
      if (MediaFile->is_Videofile())
      {
         ...
         {
            for (int j = 0; j < ParentTable->CurrentSlide->shotList[i]->ShotComposition.compositionObjects.count(); j++)
               if (ParentTable->CurrentSlide->shotList[i]->ShotComposition.compositionObjects[j]->index() == ParentTable->CompositionList->compositionObjects[index.row()]->index())
               {
                  if (ParentTable->CurrentSlide->shotList[i]->ShotComposition.compositionObjects[j]->isVisible())
                     Position += ParentTable->CurrentSlide->shotList[i]->StaticDuration;
               }
         }
         RenderImage = ((cVideoFile *)MediaFile)->ImageAt(true, Position + QTime(0, 0, 0, 0).msecsTo(((cVideoFile *)MediaFile)->StartTime), NULL, ParentTable->CompositionList->compositionObjects[index.row()]->BackgroundBrush->Deinterlace, 1, false, false);
      }
      else
         RenderImage = MediaFile->ImageAt(true);

      if (RenderImage != NULL)
      {
         // Create square workspace image
         qreal  maxw, maxh, minw, minh/*,x1,x2,x3,x4,y1,y2,y3,y4*/;
         cBrushDefinition *BackgroundBrush = compositionObject->BackgroundBrush;
         QImage *NewRenderImage = BackgroundBrush->ImageToWorkspace(RenderImage, RowHeight, maxw, maxh, minw, minh/*,x1,x2,x3,x4,y1,y2,y3,y4*/);
         QRectF CurSelRect = BackgroundBrush->appliedRect(RowHeight);
         //QRectF CurSelRect = QRectF(BackgroundBrush->X*RowHeight, BackgroundBrush->Y*RowHeight,
         //   BackgroundBrush->ZoomFactor*RowHeight, BackgroundBrush->ZoomFactor*BackgroundBrush->AspectRatio*RowHeight);
         BackgroundBrush->ApplyMaskToImageToWorkspace(NewRenderImage, CurSelRect, ParentTable->CompositionList->compositionObjects[index.row()]->BackgroundForm,
            BackgroundBrush->GetCurrentFramingStyle(ParentTable->ProjectGeometry));
   ...
   {
      if (compositionObject->tParams.TextClipArtName == "")
         Icon = ParentTable->TextIcon.copy();
      else
         Icon = TextFrameList.List[TextFrameList.SearchImage(compositionObject->tParams.TextClipArtName)].Icon.copy();
   }
   ...
   if ((ParentTable->CurrentShotNbr > 0) && (!compositionObject->hasInheritance()))
   ...
   Painter->drawImage(QRectF(option.rect.x() + 1 + RowHeight, option.rect.y() + 1 + 2 * (RowHeight / 3), 16, 16), QImage(compositionObject->isVisible() ? ICON_VISIBLE_OK : ICON_VISIBLE_KO));

   // With same as previous shot ?
   if (ParentTable->CurrentShotNbr > 0)
      Painter->drawImage(QRectF(option.rect.x() + 1 + RowHeight, option.rect.y() + 1, 16, 16), QImage(compositionObject->hasInheritance() ? ICON_INHERITDOWNUNLOCK : ICON_INHERITDOWNLOCK));

   // With sound ?
   if (compositionObject->isVideo())
   {
      double SoundVolume = ParentTable->CompositionList->compositionObjects[index.row()]->BackgroundBrush->SoundVolume;
      if (SoundVolume == -1)
         SoundVolume = 1;
   ...
   // With filter ?
   if (compositionObject->BackgroundBrush->hasFilter())
   ...
   Pen.setColor(qApp->palette().color(QPalette::Normal, QPalette::Text));
   Pen.setWidth(1);
   ...
   if (MediaFile != NULL)
   {
      ...
      pAppConfig->FilesTable->GetExtendedProperties(MediaFile->FileKey(), &TempExtProperties);
      ...
      font = getAppFont(Sans9Bold); // First line use bold
   ...
   else
   {
      ...
      TextDoc.setHtml(ParentTable->CompositionList->compositionObjects[index.row()]->tParams.Text);
   ...
   if (!ParentTable->CompositionList->compositionObjects[index.row()]->isVisible())
   ...
   if ((ParentTable->IsDragOn != 0) && (ParentTable->DragItemDest != ParentTable->DragItemSource) && (ParentTable->DragItemDest != ParentTable->DragItemSource + 1) && ((index.row() == ParentTable->DragItemDest) || ((index.row() == ParentTable->rowCount() - 1) && (index.row() == ParentTable->DragItemDest - 1))))
   {
      ...
      Painter->setOpacity(0.5);
      if (index.row() == ParentTable->DragItemDest)
         Painter->drawLine(0, option.rect.y() + 3, option.rect.width(), option.rect.y() + 3);
      else
         Painter->drawLine(0, option.rect.y() + option.rect.height() - 3, option.rect.width(), option.rect.y() + option.rect.height() - 3);
      Painter->setOpacity(1);
      Painter->restore();
   }
...
void cCustomBlockTable::getSelectedIndices(QList<int> &indexList)
{
   ...
   if (CompositionList)
   {
      ...
      {
         if (SelList.at(i).row() < CompositionList->compositionObjects.count())
            indexList.append(CompositionList->compositionObjects[SelList.at(i).row()]->index());
      }
   }
}

void cCustomBlockTable::selectIndices(const QList<int> &indexList)
{
   clearSelection();
   for (int i = 0; i < CompositionList->compositionObjects.count(); i++)
      if( indexList.contains(CompositionList->compositionObjects[i]->index()) )
      {
