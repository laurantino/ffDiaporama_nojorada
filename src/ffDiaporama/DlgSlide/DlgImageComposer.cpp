...
DlgImageComposer::DlgImageComposer(cDiaporama *ffdProject, cApplicationConfig *ApplicationConfig, QWidget *parent) :
   cShotComposer(ffdProject->ProjectThumbnail, ApplicationConfig, parent), ui(new Ui::DlgImageComposer) 
{
   ui->setupUi(this);
   ...
   NbrSelected = 0;
   CompositionList = &CurrentSlide->shotList[0]->ShotComposition;
   BlockTable->CompositionList = &CurrentSlide->shotList[0]->ShotComposition;
...
void DlgImageComposer::RefreshControls(bool UpdateInteractiveZone) {
   ...
   {
      ui->actionEditImage->setIcon(QIcon(!CurrentCompoObject->BackgroundBrush->MediaObject->is_Videofile() ? ICON_EDIT_IMAGE : ICON_EDIT_MOVIE));
...
void DlgImageComposer::s_BlockTable_Copy() {
   ...
   int          BlockNum = 0;
   for (int i = 0; i < CompositionList->compositionObjects.count(); i++) if (IsSelected[i])
   {
      ...
      CompositionList->compositionObjects[i]->SaveToXML(Element, "CLIPBOARD-BLOCK-SHOT", ffdProject->ProjectFileName, true, true, NULL, NULL, true, false);     // Save shot object
   ...
   if (SlideData->hasFormat("ffDiaporama/block"))
   {
      ...
      if ((Object.elementsByTagName("CLIPBOARD").length() > 0) && (Object.elementsByTagName("CLIPBOARD").item(0).isElement() == true))
      {
         ...
         for (int BlockNum = 0; BlockNum < BlockNbr; BlockNum++) if ((root.elementsByTagName(QString("Block-%1").arg(BlockNum)).length()>0) && (root.elementsByTagName(QString("Block-%1").arg(BlockNum)).item(0).isElement() == true))
         {
            ...
            {
               cCompositionObject *obj = new cCompositionObject(eCOMPOSITIONTYPE_SHOT, CurrentSlide->NextIndexKey, ApplicationConfig, &CurrentSlide->shotList[i]->ShotComposition);
               CurrentSlide->shotList[i]->ShotComposition.compositionObjects.append(obj);
               obj->CopyFromCompositionObject(&ShotBlock);
            }
            // Inc NextIndexKey
...
void DlgImageComposer::s_BlockTable_AddNewSimpleTextBlock()
{
   ...
   // Create and append a composition block to the object list
   cCompositionObject *CompositionObject = new cCompositionObject(eCOMPOSITIONTYPE_OBJECT, CurrentSlide->NextIndexKey, ApplicationConfig, &CurrentSlide->ObjectComposition);
   CurrentSlide->ObjectComposition.compositionObjects.append(CompositionObject);
   ...
   CompositionObject->BackgroundBrush->setAspectRatio( (CompositionObject->height()*(ffdProject->ImageGeometry == GEOMETRY_4_3 ? 1440 : ffdProject->ImageGeometry == GEOMETRY_16_9 ? 1080 : ffdProject->ImageGeometry == GEOMETRY_40_17 ? 816 : 1920)) / (CompositionObject->width() * 1920));

   // Create default text
   QTextDocument       TextDoc(QApplication::translate("DlgImageComposer", "Text", "Default text value"));
   QFont               Font = QFont(CompositionObject->tParams.FontName, CompositionObject->tParams.FontSize, CompositionObject->tParams.IsBold ? QFont::Bold : QFont::Normal, CompositionObject->tParams.IsItalic ? QFont::StyleItalic : QFont::StyleNormal);
   ...
   for (int i = 0; i < CurrentSlide->shotList.count(); i++)
   {
      cCompositionObject *obj = new cCompositionObject(eCOMPOSITIONTYPE_SHOT, CompositionObject->index(), ApplicationConfig, &CurrentSlide->shotList[i]->ShotComposition);
      CurrentSlide->shotList[i]->ShotComposition.compositionObjects.append(obj);
      obj->CopyFromCompositionObject(CompositionObject);
...
void DlgImageComposer::s_BlockTable_AddFilesBlock(QStringList FileList, int PositionToInsert) 
{
   ...
   for (int i = 0; i < FileList.count(); i++)
   {
      ...
      // Create and append a composition block to the object list                                                                                                               
      cCompositionObject  *CompositionObject = new cCompositionObject(eCOMPOSITIONTYPE_OBJECT, CurrentSlide->NextIndexKey, ApplicationConfig, &CurrentSlide->ObjectComposition);
      CurrentSlide->ObjectComposition.compositionObjects.insert(PositionToInsert, CompositionObject);
      ...
      if (IsValide)
      {
         ...
         if (CurrentBrush->MediaObject->is_Videofile())
         {
            ...
            CurrentBrush->Deinterlace = (ApplicationConfig->Deinterlace) && (Video) && ((Extension=="mts") || (Extension=="m2ts") || (Extension=="mod") || (Extension=="ts"));
         }
         ...
         if (CurrentBrush->MediaObject->is_Imagevector())
         {
            ...
            CompositionObject->ApplyAutoCompoSize(ApplicationConfig->DefaultBlockBA[CurrentBrush->GetImageType()].AutoCompo, ffdProject->ImageGeometry);

         // Now create and append a shot composition block to all shot
         for (int i = 0; i < CurrentSlide->shotList.count(); i++)
         {
            cCompositionObject *obj = new cCompositionObject(eCOMPOSITIONTYPE_SHOT, CompositionObject->index(), ApplicationConfig, &CurrentSlide->shotList[i]->ShotComposition);
            CurrentSlide->shotList[i]->ShotComposition.compositionObjects.insert(PositionToInsert, obj );
            obj->CopyFromCompositionObject(CompositionObject);
         }
         // Inc NextIndexKey
         CurrentSlide->NextIndexKey++;
...
void DlgImageComposer::s_BlockTable_DragDropFiles(QList<QUrl> urlList) {
   ...
   if (urlList.count() > 0)
   {
      ...
      if (FileList.count()>0)
      {
         if (BlockTable->DragItemDest < 0)
            BlockTable->DragItemDest = 0;
         if (BlockTable->DragItemDest > BlockTable->rowCount()) 
            BlockTable->DragItemDest = BlockTable->rowCount();
...
void DlgImageComposer::s_BlockSettings_ImageEditCorrect() 
{
   ...
   if ((InRefreshControls) || (BlockSelectMode != SELECTMODE_ONE) || (!CurrentCompoObject) || (!ui->actionEditImage->isEnabled())) 
      return;
   ...
   int Position = CurrentBrush->isVideo() ? QTime(0, 0, 0, 0).msecsTo(((cVideoFile*)&CurrentBrush->MediaObject)->StartTime) : 0;
