...
//********************************************************************************************************************************
// cShotTableItemDelegate : Item delegate for shot table
//********************************************************************************************************************************
...
void cShotTableItemDelegate::paint(QPainter *Painter, const QStyleOptionViewItem &option, const QModelIndex &index) const 
{
   ...
   {
      qDebug() << "ShotTable rowHeight " << Height;
      lastHeight = Height;
   }
   ...
   if ((CurCol >= 0) && (CurCol < ParentTable->DiaporamaObject->shotList.count()))
   {
      // Calc start position of this shot
      for (int i = 1; i <= CurCol; i++) Position = Position + ParentTable->DiaporamaObject->shotList[i - 1]->StaticDuration;

      // Calc duration of this shot
      Duration = ParentTable->DiaporamaObject->shotList[CurCol]->StaticDuration;

      // Define display color of duration (Red only if shot is the last and Position+Duration < DiaporamaObject's duration
      RedColor = ((CurCol == ParentTable->DiaporamaObject->shotList.count() - 1) && (Position + Duration < ParentTable->DiaporamaObject->GetDuration()));
   ...
   if ((ParentTable->IsDragOn == 1) && (CurCol != ParentTable->DragItemSource) && ((CurCol == ParentTable->DragItemDest) || ((CurCol == ParentTable->DragItemDest - 1) && (ParentTable->DragItemDest == ParentTable->DiaporamaObject->shotList.count()))))
   {
      TempPainter.save();
      QPen Pen;
      Pen.setColor(Qt::red);
   ...
   int   FontFactor = ((ParentTable->DiaporamaObject->pDiaporama->ApplicationConfig()->TimelineHeight - TIMELINEMINHEIGHT) / 20) * 10;
