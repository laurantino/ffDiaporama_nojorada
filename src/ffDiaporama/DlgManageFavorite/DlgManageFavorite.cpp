...
void DlgManageFavorite::PopulateList(QString ActiveName) 
{
   ...
   for (int i = 0; i < Collection->count(); i++)
   {
      ...
      if (ActiveName == Text)
         ui->ListFavorite->setCurrentRow(ui->ListFavorite->count() - 1);
   }
...
void DlgManageFavorite::s_DBRename() 
{
   ...
   if (Item)
   {
      ...
      Texts[1].chop(1);
      int i = Collection->indexOf(Texts[0] + "###" + Texts[1]);
      if (i < Collection->count()) 
      {
         ...
         while (Continue)
         {
            ...
            if (Ok && !Text.isEmpty())
            {
               ...
               int j = 0;
               QString preString(Texts[0] + "###");
               //while ((j < Collection->count()) && ((j == i) || (!(*Collection)[j].startsWith(Texts[0] + "###"))))
               while ((j < Collection->count()) && ((j == i) || (!Collection->at(j).startsWith(preString))))
                     j++;
               if (j < Collection->count() && Collection->at(j).startsWith(preString))
               {
               ...
               else
               {
                  ...
                  (*Collection)[i] = preString + Texts[1];
               }
            }
         }
