...
#include "FFDMultimediaBrowser.h"
#include "ui_FFDMultimediaBrowser.h"
#include "DlgInfoFile/DlgInfoFile.h"
#include "DlgManageFavorite/DlgManageFavorite.h"

void splitDuration(int64_t duration, int &msecs, int &seconds, int &minutes, int &hours, int &days);
//{
//   msecs = duration % 1000;
...
FFDMultimediaBrowser::FFDMultimediaBrowser(QWidget *parent) :QWidget(parent), ui(new Ui::FFDMultimediaBrowser) 
{
   ui->setupUi(this);
...
FFDMultimediaBrowser::~FFDMultimediaBrowser() 
{
...
void FFDMultimediaBrowser::DoInitWidget(BROWSER_TYPE_ID BrowserType, bool AllowMultipleSelection, bool AllowDragDrop, bool AllowAddToProject, cApplicationConfig *ApplicationConfig) 
{
...
void FFDMultimediaBrowser::DoInitDialog() 
{
   // Button bar events
...
int             FFDMultimediaBrowser::GetSortOrder()                                                    { return ui->FolderTable->SortFile; }
QStringList     FFDMultimediaBrowser::GetCurrentSelectedFiles()                                         { return ui->FolderTable->GetCurrentSelectedFiles(); }
void            FFDMultimediaBrowser::GetCurrentSelectedMediaFile(QList<cBaseMediaFile*> *SelMediaList) { ui->FolderTable->GetCurrentSelectedMediaFile(SelMediaList); }
cBaseMediaFile  *FFDMultimediaBrowser::GetCurrentMediaFile()                                            { return ui->FolderTable->GetCurrentMediaFile(); }
QSplitter       *FFDMultimediaBrowser::GetSplitter()                                                    { return ui->BrowserWidget; }
void            FFDMultimediaBrowser::EnsureThreadIsStopped()                                           { ui->FolderTable->EnsureThreadIsStopped(); }

//====================================================================================================================

void FFDMultimediaBrowser::RefreshControls(bool EmitToParent) 
{
...
void FFDMultimediaBrowser::FolderTreeItemChanged(QTreeWidgetItem *current, QTreeWidgetItem *) 
{
...
#ifdef Q_OS_WIN
   Path.replace("%HOMEDRIVE%%HOMEPATH%",ApplicationConfig->DriveList->drives[0].Path,Qt::CaseInsensitive);
   Path.replace("%USERPROFILE%",ApplicationConfig->DriveList->drives[0].Path,Qt::CaseInsensitive);
...
void FFDMultimediaBrowser::DoBrowserRefreshFolderInfo() 
{
   ...
   if (HDD)
   {
      ...
      else
      {
         ...
         int msec, sec, mn, hours, days;
         splitDuration(duration, msec, sec, mn, hours, days);
         if (days > 0) 
            ui->FolderDuration->setText(QString("%1.%2:%3:%4.%5").arg(ito2a(days)).arg(ito2a(hours)).arg(ito2a(mn)).arg(ito2a(sec)).arg(ito3a(msec)));
         else    
            ui->FolderDuration->setText(QString("%1:%2:%3.%4").arg(ito2a(hours)).arg(ito2a(mn)).arg(ito2a(sec)).arg(ito3a(msec)));
      }
...
void FFDMultimediaBrowser::DoBrowserRefreshSelectedFileInfo() 
{
   if (ui->FolderTable->InSelChange) 
      return;
   ...
   else if (MediaList.count() == 1)
   {
      ...
      if (FStr != "") 
         ui->FileInfo1a->setText(QString("%1 (%2)").arg(MediaObject->ShortName()).arg(FStr));
      else 
         ui->FileInfo1a->setText(MediaObject->ShortName());
      if (QTime(0, 0, 0, 0).msecsTo(MediaObject->GetRealDuration()) > 0) 
         ui->FileInfo2a->setText(QString("%1-%2").arg(MediaObject->GetTechInfo(&TempExtProperties)).arg(MediaObject->GetRealDuration().toString("HH:mm:ss.zzz")));
      else 
         ui->FileInfo2a->setText(MediaObject->GetTechInfo(&TempExtProperties));
   ...
   else if (MediaList.count() > 1)
   {
      ...
      for (int i = 0; i < MediaList.count(); i++)
      {
         ...
         if (MediaList[i]->is_Musicfile() || MediaList[i]->is_Videofile() || MediaList[i]->is_FFDfile())
      ...
      {
         int msec, sec, mn, hours, days;
         splitDuration(TotalDuration, msec, sec, mn, hours, days);
         if (days>0) 
            ui->FileInfo2a->setText(QApplication::translate("MainWindow", "Total duration:") + QString("%1.%2:%3:%4.%5").arg(ito2a(days)).arg(ito2a(hours)).arg(ito2a(mn)).arg(ito2a(sec)).arg(ito3a(msec)));
         else    
            ui->FileInfo2a->setText(QApplication::translate("MainWindow", "Total duration:") + QString("%1:%2:%3.%4").arg(ito2a(hours)).arg(ito2a(mn)).arg(ito2a(sec)).arg(ito3a(msec)));
      }
      else 
         ui->FileInfo2a->setText("");

      if (TotalSize != 0) 
         ui->FileInfo3a->setText(QApplication::translate("MainWindow", "Total size:%1").arg(GetTextSize(TotalSize)));
      else          
         ui->FileInfo3a->setText("");
...
void FFDMultimediaBrowser::SetToPrevious() 
{
...
void FFDMultimediaBrowser::SetToUpper() 
{
...
void FFDMultimediaBrowser::RefreshAll() 
{
   CurrentDriveCheck = 0;
   CancelAction = false;
   ...
   DlgWorkingTaskDialog->SetMaxValue(ApplicationConfig->DriveList->drives.count() + 2, 0);
...
void FFDMultimediaBrowser::RefreshDriveList() 
{
   //autoTimer("RefreshDriveList");
   // Refresh drive list
   ui->FolderTree->RefreshDriveList();
   DlgWorkingTaskDialog->SetMaxValue(ApplicationConfig->DriveList->drives.count() + 2, 0);
   DlgWorkingTaskDialog->DisplayText(QApplication::translate("MainWindow", "update drive (%1)").arg(ApplicationConfig->DriveList->drives[CurrentDriveCheck].Label));
   DlgWorkingTaskDialog->DisplayProgress(1 + ApplicationConfig->DriveList->drives.count() - CurrentDriveCheck);
   QTimer::singleShot(LATENCY, this, SLOT(RefreshDrive()));
}

//====================================================================================================================

void FFDMultimediaBrowser::RefreshDrive() 
{
   if (CurrentDriveCheck < ApplicationConfig->DriveList->drives.count()) 
      ui->FolderTree->RefreshItemByPath(ApplicationConfig->DriveList->drives[CurrentDriveCheck].Label, true);
   CurrentDriveCheck++;
   if (!CancelAction && CurrentDriveCheck < ApplicationConfig->DriveList->drives.count())
   {
      DlgWorkingTaskDialog->DisplayText(QApplication::translate("MainWindow", "update drive (%1)").arg(ApplicationConfig->DriveList->drives[CurrentDriveCheck].Label));
      DlgWorkingTaskDialog->DisplayProgress(1 + ApplicationConfig->DriveList->drives.count() - CurrentDriveCheck);
      QTimer::singleShot(LATENCY, this, SLOT(RefreshDrive()));
...
void FFDMultimediaBrowser::RefreshHere() 
{
...
void FFDMultimediaBrowser::OpenFolder() 
{
   ...
   if (Media)
   {
      if (Media->is_Folder())
      {
         ...
         if (ui->FolderTable->BrowserType == BROWSER_TYPE_MainWindow) 
            emit DoOpenFile(); 
         else 
            emit DoAddFiles();
      delete Media;
...
void FFDMultimediaBrowser::FavoriteMenu() 
{
   ...
   if (Action)
   {
      ...
      if (ActionType == FAVACTIONTYPE_ADD)          
         AddToFavorite();
      else if (ActionType == FAVACTIONTYPE_MANAGE)  
         ManageFavorite();
      else if (Action->text() != "")
      {
...
void FFDMultimediaBrowser::AddToFavorite() 
{
   bool        Ok, Continue = true;
   QString     Text = ui->FolderTable->CurrentPath;
   QStringList BrowserFavorites = ApplicationConfig->LoadBrowserFavoritesFromDabase();

   while (Text.indexOf(QDir::separator()) != -1) 
      Text = Text.mid(Text.indexOf(QDir::separator()) + 1);
   while (Continue)
   {
      Continue = false;
      ...
      {
         int i = 0;
         while (i < BrowserFavorites.count() && !BrowserFavorites[i].startsWith(Text + "###")) 
            i++;
         ...
         else
         {
            ...
            int i = 0;
            while (i < BrowserFavorites.count() && !BrowserFavorites[i].endsWith("###" + ui->FolderTable->CurrentPath)) 
               i++;
...
void FFDMultimediaBrowser::ManageFavorite() 
{
   QStringList BrowserFavorites = ApplicationConfig->LoadBrowserFavoritesFromDabase();
...
void FFDMultimediaBrowser::ChangeToIconView() 
{
...
void FFDMultimediaBrowser::ChangeToDetailView() 
{
...
void FFDMultimediaBrowser::SettingsMenu() 
{
   // Create menu
...
bool FFDMultimediaBrowser::InRemoveFolder(QString FolderPath) 
{
   ...
   foreach(QFileInfo Dir, Dirs)
      if (Dir.isDir() && Dir.absoluteFilePath() != FolderPath && Dir.fileName() != "." && Dir.fileName() != "..")
         if (!InRemoveFolder(Dir.absoluteFilePath())) 
            return false;
...
void FFDMultimediaBrowser::RemoveFolder() 
{
   ...
   if (NewFolderPath.lastIndexOf(QDir::separator()) != -1) 
      NewFolderPath = NewFolderPath.left(NewFolderPath.lastIndexOf(QDir::separator()));
...
void FFDMultimediaBrowser::RenameFolder() 
{
   bool    Ok;
   bool    PersoF = false;
   ...
   if (SubFolderName.endsWith(QDir::separator())) 
      SubFolderName = SubFolderName.left(SubFolderName.length() - 1);
   if (SubFolderName.indexOf(QDir::separator()) != -1)
   {
      ...
      {
         if (!QDir().rename(SrcFolder, FolderPath + QDir::separator() + SubFolderName)) 
            CustomMessageBox(this, QMessageBox::Critical, QApplication::translate("MainWindow", "Rename folder"), QApplication::translate("MainWindow", "Impossible to rename folder!"), QMessageBox::Ok);
         else
         {
            if (PersoF)
            {
...
void FFDMultimediaBrowser::RemoveFile() 
{
   QList<cBaseMediaFile*> MediaList;
   ui->FolderTable->GetCurrentSelectedMediaFile(&MediaList);
   if (MediaList.count() > 0)
   {
      ...
      for (int i = 0; i < MediaList.count(); i++)
      {
         ...
         if (!FileExtension.isEmpty())                   
            FName.replace("." + FileExtension, "");
         if (QFileInfo(FName + ".jpg").exists())           
            FileList.append(FName + ".jpg");
         if (QFileInfo(FName + "-poster.jpg").exists())    
            FileList.append(FName + "-poster.jpg");
         if (QFileInfo(FName + ".nfo").exists())           
            FileList.append(FName + ".nfo");
      }
...
void FFDMultimediaBrowser::s_Action_DoRemoveFile() 
{
   ...
   if (CustomMessageBox(this, QMessageBox::Question, APPLICATION_NAME, QApplication::translate("MainWindow", "Are you sure to delete this file or folder ?\n(Warning: Content will not be moved to trash)") + "\n" + FileToRemove, QMessageBox::Yes | QMessageBox::No) == QMessageBox::Yes)
   {
      ...
      {
         if (!QDir().rmdir(FileToRemove)) 
            CustomMessageBox(this, QMessageBox::Critical, QApplication::translate("MainWindow", "Remove folder"), QApplication::translate("MainWindow", "Impossible to delete folder!\nAre you sure it is empty?"), QMessageBox::Ok);
      }
      else
      {
         if (!QFile(FileToRemove).remove()) 
            CustomMessageBox(this, QMessageBox::Critical, QApplication::translate("MainWindow", "Remove file"), QApplication::translate("MainWindow", "Impossible to delete file!"), QMessageBox::Ok);
      }
   }
   QTimer::singleShot(LATENCY, this, SLOT(s_Action_DoRemoveFile()));
}
...
void FFDMultimediaBrowser::RenameFile() 
{
   cBaseMediaFile *Media = ui->FolderTable->GetCurrentMediaFile();
   if (Media)
   {
