...
QCustomFolderTable::QCustomFolderTable(QWidget *parent):QTableWidget(parent) 
{
   ...
   connect(this, SIGNAL(callUpdate(const QModelIndex&)), this, SLOT(onCcallUpdate(const QModelIndex&)));
}
...
QString QCustomFolderTable::GetTextForColumn(int Col,cBaseMediaFile *MediaObject,QStringList *ExtendedProperties) 
{
    ...
    switch (horizontalHeaderItem(Col)->data(Qt::UserRole).toInt())
    {
    ...
    //if      (ColName == tr("File","Column header"))             TextToDisplay = MediaObject->ShortName();
    ...
    //else if (ColName == QApplication::translate("QCustomFolderTable","Title","Column header"))            TextToDisplay = !MediaObject->is_FFDfile() ? GetInformationValue("title",ExtendedProperties) : ((cffDProjectFile *)MediaObject)->Title();
    //else if (ColName == QApplication::translate("QCustomFolderTable","Artist","Column header"))           TextToDisplay = !MediaObject->is_FFDfile() ? GetInformationValue("artist",ExtendedProperties) : ((cffDProjectFile *)MediaObject)->Author();
    //else if (ColName == QApplication::translate("QCustomFolderTable","Author","Column header"))           TextToDisplay = !MediaObject->is_FFDfile() ? GetInformationValue("artist",ExtendedProperties) : ((cffDProjectFile *)MediaObject)->Author();
    //else if (ColName == QApplication::translate("QCustomFolderTable","Album","Column header"))            TextToDisplay = !MediaObject->is_FFDfile() ? GetInformationValue("album",ExtendedProperties) : ((cffDProjectFile *)MediaObject)->Album();
    //else if (ColName == QApplication::translate("QCustomFolderTable","Year","Column header"))             TextToDisplay = !MediaObject->is_FFDfile() ? GetInformationValue("date",ExtendedProperties)  : ((cffDProjectFile *)MediaObject)->EventDate().toString(ApplicationConfig->ShortDateFormat);
    ...
    //else if (ColName == QApplication::translate("QCustomFolderTable","Comment","Column header"))          TextToDisplay = !MediaObject->is_FFDfile() ? GetInformationValue("comment",ExtendedProperties) : ((cffDProjectFile *)MediaObject)->Comment();
    //else if (ColName == QApplication::translate("QCustomFolderTable","Composer","Column header"))         TextToDisplay = !MediaObject->is_FFDfile() ? GetInformationValue("composer",ExtendedProperties) : ((cffDProjectFile *)MediaObject)->Composer();
...
int QCustomFolderTable::GetAlignmentForColumn(COLTYPE Col)
{
   if (StopAllEvent )
      return Qt::AlignLeft;
...
bool QCustomFolderTable::setThreadToWait()
{
   if (ScanMediaList.isRunning()) 
   {
      ScanMediaList.pause();
...
bool ByNumber(const MediaFileItem &Item1,const MediaFileItem &Item2) 
{
   ...
   if (ok1 && ok2) 
      return NumA < NumB; 
   return Item1.ShortName.compare(Item2.ShortName, Qt::CaseInsensitive) < 0 ;
...
void QCustomFolderTable::FillListFolder(QString Path) 
{
   ...
   if (!ShowHiddenFilesAndDir) 
      QueryString = QueryString+" AND IsHidden=0";
   ...
   {
      bool Ret = true;
      qlonglong FileKey = Query.value(0).toLongLong(&Ret);
      if (!Ret) 
         FileKey = -1;
      OBJECTTYPE ObjectType = (OBJECTTYPE)Query.value(1).toInt(&Ret);
      if (!Ret) 
      ...
      if (FileKey != -1 && ObjectType != -1) 
      {
         ...
         if (MediaObject) 
         {
            ...
            if (CurrentMode == DISPLAY_DATA) 
            {
               ...
               for (int Col = 0; Col < columnCount(); Col++) 
                  MediaList[MediaList.count()-1].TextToDisplay.append(GetTextForColumn(Col,MediaObject,&ExtendedProperties));
            } 
            else 
            {
   ...
   bool (*lessFunc)(const MediaFileItem&, const MediaFileItem&) = NULL;
   if(SortFile == SORTORDER_BYNUMBER)
      ...
      lessFunc = ByDate;
   if( lessFunc )
      QT_QSORT(MediaList.begin(),MediaList.end(),lessFunc);
   ...
   ScanMediaList.setFuture(QT_CONCURRENT_RUN(this,&QCustomFolderTable::DoScanMediaList));
   ...
   for (int ItemIndex = 0; ItemIndex < MediaList.count() && !StopScanMediaList && !StopAllEvent;ItemIndex++) 
   {
      if (!MediaList[ItemIndex].IsInformationValide) 
      {
         ...
         if (CurrentMode == DISPLAY_DATA) 
         {
            ...
            for (int Col = 0; Col < columnCount(); Col++) 
               emit callUpdate(model()->index(ItemIndex, Col)); //update(model()->index(ItemIndex,Col));
         } 
         else 
         {
            ...
            //update(model()->index(Row,Col));
            emit callUpdate(model()->index(Row, Col));
