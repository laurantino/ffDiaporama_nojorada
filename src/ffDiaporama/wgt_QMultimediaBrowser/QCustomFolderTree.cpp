...
void QCustomFolderTree::s_ContextMenu(const QPoint) 
{
   ...
   if (IsCreateFolderAllowed)  
      ContextMenu->addAction(CreateMenuAction(QApplication::style()->standardIcon(QStyle::SP_FileDialogNewFolder),QApplication::translate("QCustomFolderTree","Create new subfolder"),3,false,false,this));
   if (IsRemoveAllowed)        
      ContextMenu->addAction(CreateMenuAction(QApplication::style()->standardIcon(QStyle::SP_TrashIcon),          QApplication::translate("QCustomFolderTree","Remove folder"),4,false,false,this));   //":/img/trash.png"
   if (IsRenameAllowed)        
      ContextMenu->addAction(CreateMenuAction(QIcon(":/img/action_edit.png"),                                     QApplication::translate("QCustomFolderTree","Rename folder"),5,false,false,this));
...
{
   if (FilePath.startsWith(QApplication::translate("QCustomFolderTree","Clipart"))) 
      FilePath = ClipArtFolder+FilePath.mid(QApplication::translate("QCustomFolderTree","Clipart").length());
#if defined(Q_OS_LINUX) || defined(Q_OS_SOLARIS)
...
void QCustomFolderTree::s_itemExpanded(QTreeWidgetItem *item) 
{
    ...
    if (item->childCount()==1 && item->child(0)->text(0)==TAG)
    {
        ...
        if (Folder.startsWith(QApplication::translate("QCustomFolderTree","Clipart"))) 
           Folder=ClipArtFolder+Folder.mid(QApplication::translate("QCustomFolderTree","Clipart").length());
        #if defined(Q_OS_LINUX) || defined(Q_OS_SOLARIS)
...
void QCustomFolderTree::SetSelectItemByPath(QString Path) 
{
   ...
   Folders = Path.split(QChar('/'), Qt::SkipEmptyParts);
    ...
    if ((Folders.count() >= 1) && (Folders[0] == "")) 
      Folders[0] = QApplication::translate("QCustomFolderTree","System files");
    #endif
    ...
    while (Current != NULL && i < Folders.count() && Folders[i] != "") 
    {
       ...
       if (Current) 
       {
          ...
          { // If not initialize
             ...
             if (RealPath.startsWith(QApplication::translate("QCustomFolderTree","Clipart"))) 
               RealPath = ClipArtFolder + RealPath.mid(QApplication::translate("QCustomFolderTree","Clipart").length());
#if defined(Q_OS_LINUX) || defined(Q_OS_SOLARIS)
...
void QCustomFolderTree::RefreshDriveList() {
    ...
    while (i<ApplicationConfig->DriveList->drives.count()) {
        if (ApplicationConfig->DriveList->drives[i].Label==QApplication::translate("QCustomFolderTree","Clipart"))
        ...
        if (ApplicationConfig->DriveList->drives[i].Flag==0) {
            ...
            if (ApplicationConfig->DriveList->drives[i].Path.startsWith("/media/")
                ...
                ||(ApplicationConfig->DriveList->drives[i].Label==QApplication::translate("QCustomFolderTree","Clipart"))
