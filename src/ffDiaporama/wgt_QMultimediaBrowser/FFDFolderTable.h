...
#ifndef FFDFOLDERTABLE_H
#define FFDFOLDERTABLE_H
...
//*************************************************************
class MediaFileItem ;
class FFDFolderTable : public QTableWidget 
{
   Q_OBJECT
enum COLTYPE
{
   eFILE, eFILE_TYPE, eFILE_SIZE,eFILE_DATE,
   eDURATION,
   ...
   eTITLE, eARTIST, eAUTHOR, eALBUM, eYEAR, eTRACK, eGENRE, eCOMMENT, eCOMPOSER, eENCODER, eCHAPTERS
};
   public:
   ...
   public:
      explicit FFDFolderTable(QWidget *parent = 0);
      ~FFDFolderTable();
    ...
    virtual int             GetAlignmentForColumn(COLTYPE Col);
   ...
   signals:
      ...
      void RenameFiles();
      void Refresh();
      void refreshData(int row, int col);
   ...
   QMap<QString, qlonglong> cachedKeys;
   qint64 chachedFolder;
   void createAndCacheKeys(qint64 FolderKey, const QFileInfoList &fileList);
   qlonglong GetCachedFileKey(qlonglong FolderKey,QString ShortName);
friend class FFDStyledItemDelegate;
};

#endif // FFDFOLDERTABLE_H
