...
void DlgffDPjrProperties::DoInitDialog() {
    ...
    QStringList const *props = ffdProject->ProjectInfo()->chapterProps();
    for (int i=0;(i==0)||(i<ffdProject->ProjectInfo()->numChapters());i++) {
        int     iChapterNum=ffdProject->ProjectInfo()->numChapters()==0?0:i+1;
        ...
        ui->ChapterTable->setItem(ui->ChapterTable->rowCount()-1,1,CreateItem(GetInformationValue(ChapterNum+"InSlide", props), Qt::AlignLeft|Qt::AlignVCenter,Background));
        ui->ChapterTable->setItem(ui->ChapterTable->rowCount()-1,2,CreateItem(GetInformationValue(ChapterNum+"title",   props), Qt::AlignLeft|Qt::AlignVCenter,Background));
        ui->ChapterTable->setItem(ui->ChapterTable->rowCount()-1,3,CreateItem(GetInformationValue(ChapterNum+"Start",   props), Qt::AlignLeft|Qt::AlignVCenter,Background));
        ui->ChapterTable->setItem(ui->ChapterTable->rowCount()-1,4,CreateItem(GetInformationValue(ChapterNum+"End",     props), Qt::AlignLeft|Qt::AlignVCenter,Background));
        ui->ChapterTable->setItem(ui->ChapterTable->rowCount()-1,5,CreateItem(GetInformationValue(ChapterNum+"Duration",props), Qt::AlignLeft|Qt::AlignVCenter,Background));
    }
    ...
    if (ffdProject->ProjectInfo()->Location()) {
        ui->Location->setText(QString("%1 (%2)").arg(ffdProject->ProjectInfo()->Location()->Name).arg(ffdProject->ProjectInfo()->Location()->FriendlyAddress));
        ui->LocationIcon->setPixmap(QPixmap().fromImage(ffdProject->ProjectInfo()->Location()->GetThumb(16)));
    } else {
        ui->Location->setText("");
...
bool DlgffDPjrProperties::DoAccept() {
    if (IsPrjCreate) 
      ffdProject->ImageGeometry = (ffd_GEOMETRY)ui->GeometryCombo->currentIndex();
    ffdProject->ProjectInfo()->setAlbum(ui->AlbumED->text());
    ffdProject->ProjectInfo()->setAuthor(ui->AuthorED->text());
    ffdProject->ProjectInfo()->setLongDate(ui->DateED->toPlainText());
    ffdProject->ProjectInfo()->setEventDate(ui->DateEdit->date());
    ffdProject->ProjectInfo()->setOverrideDate(ui->OverrideDateCB->isChecked());
    ffdProject->ProjectInfo()->setComment(ui->CommentED->toPlainText());
    ffdProject->ProjectInfo()->setDefaultLanguage(ui->LanguageED->text());
    ...
    if (IsLocationChanged && AllowGMapRefresh)
      ffdProject->UpdateGMapsObject();
...
void DlgffDPjrProperties::TitleChanged(QString) {
    ffdProject->ProjectInfo()->setTitle(ui->TitleED->text());

    ffdProject->clearResolvedTexts();
    ApplicationConfig->ImagesCache.RemoveObjectImages();

    ui->ThumbCB->MakeIcons();    // Refresh icons for custom combo
}
...
void DlgffDPjrProperties::EventDateChanged(const QDate &NewDate) {
    ffdProject->ProjectInfo()->setEventDate(NewDate);
    if (!ffdProject->ProjectInfo()->OverrideDate()) {
        ffdProject->ProjectInfo()->setLongDate(FormatLongDate(ffdProject->ProjectInfo()->EventDate()));
        ui->DateED->setPlainText(ffdProject->ProjectInfo()->LongDate());
    }
    ui->ThumbCB->MakeIcons();    // Refresh icons for custom combo
}
...
void DlgffDPjrProperties::SelectLocation() {
    ...
    if (!Query.exec()) DisplayLastSQLError(&Query); else while (Query.next()) {
        ...
        if (Ret) {
            ...
            bool        IsCurrent = (Key == (ffdProject->ProjectInfo()->Location() ? ffdProject->ProjectInfo()->Location()->FavKey : -1));
    ...
    if (Action) 
    {
       ...
       {
          cLocation *PrevLocation = ffdProject->ProjectInfo()->Location();
          if (!PrevLocation) 
            ffdProject->ProjectInfo()->createLocation();
          DlgGMapsLocation Dlg(ffdProject->ProjectInfo()->Location(),ApplicationConfig,this);
          Dlg.InitDialog();
          if (Dlg.exec() != 0 && !PrevLocation) 
          {
             ffdProject->ProjectInfo()->clearLocation();
          }
       } 
       else if (Action->text() != "") 
       {
          ...
          if (!ffdProject->ProjectInfo()->Location()) 
             ffdProject->ProjectInfo()->createLocation();
          ffdProject->ProjectInfo()->Location()->LoadFromFavorite(Key);
       }
    }
