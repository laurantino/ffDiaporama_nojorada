...
void cLocation::SaveToXMLex(ExXmlStreamWriter &xmlStream, QString ElementName, QString PathForRelativPath, bool ForceAbsolutPath, cReplaceObjectList *ReplaceList, QList<qlonglong> *ResKeyList, bool IsModel)
{
   xmlStream.writeAttribute("LocationType", LocationType);
   if ((!IsModel) || (IsModel && (LocationType == FREE)))
   {
      xmlStream.writeAttribute("Name", Name);
      xmlStream.writeAttribute("FAddress", FriendlyAddress);
      xmlStream.writeAttribute("GPS_cx", GPS_cx);
      xmlStream.writeAttribute("GPS_cy", GPS_cy);
   }
   if (LocationType == FREE)
   {
      xmlStream.writeAttribute("Address", Address);
      xmlStream.writeAttribute("ZoomLevel", ZoomLevel);
      //Icon->SaveToXMLEx(xmlStream, "Icon", PathForRelativPath, ForceAbsolutPath, ReplaceList, ResKeyList, false);
      if (ResKeyList) 
         ResKeyList->append(ThumbnailResKey);
      xmlStream.writeAttribute("ThumbResKey", ThumbnailResKey);
   }
   xmlStream.writeAttribute("MarkerW", MarkerSize.width());
   xmlStream.writeAttribute("MarkerH", MarkerSize.height());
   if (Size != DEFAULT_Size)                         xmlStream.writeAttribute("Size", int(Size));
   if (Distance != DEFAULT_Distance)                 xmlStream.writeAttribute("Distance", int(Distance));
   if (MarkerPointForm != DEFAULT_MarkerPointForm)   xmlStream.writeAttribute("MarkerPointForm", int(MarkerPointForm));
   if (MarkerCompo != DEFAULT_MarkerCompo)           xmlStream.writeAttribute("MarkerCompo", int(MarkerCompo));
   if (MarkerForm != DEFAULT_MarkerForm)             xmlStream.writeAttribute("MarkerForm", int(MarkerForm));
   if (LocationType == FREE)
      Icon->SaveToXMLex(xmlStream, "Icon", PathForRelativPath, ForceAbsolutPath, ReplaceList, ResKeyList, false);
}
...
bool cLocation::LoadFromXML(QDomElement *ParentElement,QString,QString PathForRelativPath,QStringList *AliasList,bool *ModifyFlag,TResKeyList *ResKeyList,bool DuplicateRes) {
    bool IsOk=true;
    ...
    if (LocationType==FREE) {
        ...
        if ((IsOk)&&(ThumbnailResKey==-1)) {
            QImage ThumbImage;
            if (Icon->MediaObject) {
                ThumbImage=Icon->GetImageDiskBrush(QSizeF(64,64),true,0,NULL,1,NULL);
                Icon->ApplicationConfig->SlideThumbsTable->SetThumb(&ThumbnailResKey,ThumbImage);
            } else {
            ...
            {
                QImage ThumbImage;
                Icon->ApplicationConfig->SlideThumbsTable->GetThumb(&ThumbnailResKey,&ThumbImage);
                ThumbnailResKey = -1;
                Icon->ApplicationConfig->SlideThumbsTable->SetThumb(&ThumbnailResKey,ThumbImage);
            }
        }
...
void cLocation::AddToFavorite() {
    QImage  Image;
    if (Icon->MediaObject) Icon->GetImageDiskBrush(QSizeF(64,64),true,0,NULL,1,NULL); else {
    ...
    while (Query.next()) {
        ...
        if (domDocument.setContent(sIcon,true,&errorStr,&errorLine,&errorColumn)) {
            if ((domDocument.elementsByTagName("Icon").length()>0)&&(domDocument.elementsByTagName("Icon").item(0).isElement()==true)) {
                ...
                QImage ThumbImage;
                if (Thumb.isNull()) {
                    ThumbImage=Icon->GetImageDiskBrush(QSizeF(64,64),false,0,NULL,1,NULL);
                    if (Icon->MediaObject) Icon->GetImageDiskBrush(QSizeF(64,64),true,0,NULL,1,NULL); else {
                        ThumbImage=QImage(64,64,QImage::Format_ARGB32_Premultiplied);
                        ThumbImage.fill(Qt::white);
                    }
...
QImage cLocation::GetThumb(int IconSize) {
    ...
    if (Icon->MediaObject) {
        ...
        if (Thumb.isNull()) {
            Thumb=Icon->GetImageDiskBrush(QSizeF(64,64),false,0,NULL,1,NULL);
            if (!Thumb.isNull()) {
                Icon->ApplicationConfig->SlideThumbsTable->SetThumb(&ThumbnailResKey,Thumb);
