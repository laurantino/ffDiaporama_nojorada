   ...
   if (!httpstatuscode) 
   {
      if (GetNewtorkDataReply->error() == QNetworkReply::NoError) 
      {
         ...
         if(TimeOutTimer.isActive()) 
         {
            TimeOutTimer.stop();
            if (Status) 
               ToLog(LOGMSG_INFORMATION,QString("Downloading %1 from ffDiaporama Web Site ... done").arg(NetworkDataFileName));
            else
               ToLog(LOGMSG_INFORMATION,QString("Downloading %1 from ffDiaporama Web Site ... error").arg(NetworkDataFileName));
         } 
         else 
         {
            // timeout
...
void DownloadObject::httpGetDataFinished() 
{
   ...
   {
      Status = (!NetworkData.contains(QString("404 - Not Found").toLocal8Bit()) && !NetworkData.contains(QString("404 Not Found").toLocal8Bit())); 
      int error = GetNewtorkDataReply->error();
...
{
   if (GetNewtorkDataReply) 
      NetworkData.append(GetNewtorkDataReply->readAll());
}

//**********************************************************************************************************************
// cApplicationConfig
//**********************************************************************************************************************
cApplicationConfig *theAppConfig = NULL;
cApplicationConfig *appConfig() { return theAppConfig; }
...
cApplicationConfig::cApplicationConfig(QMainWindow *TheTopLevelWindow,QString TheAllowedWEBLanguage):cBaseAppConfig(TheTopLevelWindow) 
{
   ...
   {
      PrjTitleModelsNextNumber[geo] = 0;    
      for (int Cat = 0; Cat < MODELTYPE_PROJECTTITLE_CATNUMBER; Cat++)  
         PrjTitleModels[geo][Cat] = NULL;
      CptTitleModelsNextNumber[geo] = 0;    
      for (int Cat = 0; Cat < MODELTYPE_CHAPTERTITLE_CATNUMBER; Cat++)  
         CptTitleModels[geo][Cat] = NULL;
      CreditTitleModelsNextNumber[geo] = 0; 
      for (int Cat = 0; Cat < MODELTYPE_CREDITTITLE_CATNUMBER; Cat++)
         CreditTitleModels[geo][Cat] = NULL;
   }

   // Initialise all variables and set them to default value
   ParentWindow            = NULL;
   ...
   //*********************************************************************
   // Search platform and define specific value depending on platform
   //*********************************************************************
#ifdef Q_OS_WIN
   // todo rewrite this
   ...
   //   case 0x00b0: Plateforme = "Windows 8.1";                                                     break;
   //   case 0x00c0: Plateforme = "Windows 10";                                                     break;
   ...
   if (UserConfigPath[UserConfigPath.length()-1] != QDir::separator()) 
      UserConfigPath = UserConfigPath + QDir::separator();
   UserConfigPath  = UserConfigPath + QString(APPLICATION_NAME) + QDir::separator();
#else
   UserConfigPath = QDir::homePath();
   if (UserConfigPath[UserConfigPath.length()-1] != QDir::separator()) 
...
#endif
   enableYUVPassthrough = true;
   enableSmoothSVG = true;
   enableCRF = true;
...
{
   if (!AllowedWEBLanguage.contains(Language)) 
      Language="en";
   return Language;
}
...
void cApplicationConfig::PreloadSystemIcons() 
{
   ...
   if (QSysInfo().productVersion().toInt() >= 7) // tocheck
   { // At least Windows 7
...
{
   //************************************
   // Prepare lists of allowed extensions
   //************************************
   AllowVideoExtension = AllowVideoExtensions.split("#");          
   AllowVideoExtension.append(AllowVideoExtensions.toUpper().split("#"));
   AllowImageExtension = AllowImageExtensions.split("#");          
   AllowImageExtension.append(AllowImageExtensions.toUpper().split("#"));
   AllowImageVectorExtension = AllowImageVectorExtensions.split("#");    
   AllowImageVectorExtension.append(AllowImageVectorExtensions.toUpper().split("#"));
   AllowMusicExtension = AllowMusicExtensions.split("#");          
   AllowMusicExtension.append(AllowMusicExtensions.toUpper().split("#"));

   //************************************
   // set language
   //************************************
   // First thing to do is to load ForceLanguage, 
   // UseNetworkProxy and NetworkProxy from USERCONFIGFILE and 
   // install AppTranslator to fix text codec !
   ...
   if (file.open(QFile::ReadOnly | QFile::Text))
   {
      ...
      if (domDocument.setContent(in.readAll(), true, &errorStr, &errorLine, &errorColumn))
      {
         ...
         {
            QDomElement Element = root.elementsByTagName("GlobalPreferences").item(0).toElement();
            if ((Element.hasAttribute("ForceLanguage")) && (ForceLanguage == "")) 
               ForceLanguage = Element.attribute("ForceLanguage");
            if (Element.hasAttribute("UseNetworkProxy"))                      
               UseNetworkProxy = Element.attribute("UseNetworkProxy") == "1";
            if (Element.hasAttribute("NetworkProxy"))                         
               NetworkProxy = Element.attribute("NetworkProxy");
            if (Element.hasAttribute("NetworkProxyPort"))                     
               NetworkProxyPort = Element.attribute("NetworkProxyPort").toInt();
            if (Element.hasAttribute("NetworkProxyUser"))                     
               NetworkProxyUser = Element.attribute("NetworkProxyUser");
            if (Element.hasAttribute("NetworkProxyPWD"))                      
               NetworkProxyPWD = Element.attribute("NetworkProxyPWD");
         }
      }
   }
   ...
   QString localeVersionFilename = UserConfigPath + QString("LOCALEVERSION.TXT");
   if ((DownloadFile("LOCALEVERSION.TXT")) && (QFileInfo(localeVersionFilename).exists()))
   {
      QFile File(localeVersionFilename);
      if (File.open(QFile::ReadOnly | QFile::Text))
      {
      ...
      // try to download version from the web site
      QString wikiVersionFilename = UserConfigPath + QString("WIKIVERSION.TXT");
      if ((DownloadFile("WIKIVERSION.TXT")) && (QFileInfo(wikiVersionFilename).exists()))
      {
         QFile File(wikiVersionFilename);
         if (File.open(QFile::ReadOnly | QFile::Text))
         {
            QTextStream InStream(&File);
#if QT_VERSION < 0x060000
   ...
   if (CurrentLanguage != "en")
   {
      ...
      QString subLanguageVersionFile = UserConfigPath + QString("%1_VERSION.TXT").arg(CurrentSubLanguage);
      QString LanguageVersionFile = UserConfigPath + QString("%1_VERSION.TXT").arg(CurrentLanguage);
      if (QFileInfo(subLanguageVersionFile).exists())
      {
         QFile File(subLanguageVersionFile);
         if (File.open(QFile::ReadOnly | QFile::Text))
         {
            QTextStream InStream(&File);
#if QT_VERSION < 0x060000
      ...
      else if (QFileInfo(LanguageVersionFile).exists())
      {
         QFile File(LanguageVersionFile);
         if (File.open(QFile::ReadOnly | QFile::Text))
         {
            QTextStream InStream(&File);
#if QT_VERSION < 0x060000
      ...
      if (QFileInfo(UserConfigPath + QString(APPLICATION_NAME) + QString("_") + QString(CurrentSubLanguage) + QString(".qm")).exists())
      {
         ...
         if (!AppTranslator.load(UserConfigPath + QString(APPLICATION_NAME) + QString("_") + CurrentLanguage + QString(".qm")))
         {
            ...
            CustomMessageBox(NULL, QMessageBox::Critical, "ffDiaporama",
               ...
               QString("Error loading application translation file ... ") + UserConfigPath + QString(APPLICATION_NAME) + QString("_") + CurrentSubLanguage + QString(".qm"),
               QMessageBox::Close);
            exit(1);
         }
         else
         ...
         {
            if (!QTtranslator.load(QString("qt_") + CurrentLanguage + QString(".qm"), UserConfigPath))
            {
      ...
      else if (QFileInfo(UserConfigPath + QString(APPLICATION_NAME) + QString("_") + QString(CurrentLanguage) + QString(".qm")).exists())
      {
         ...
         {
            if (!QTtranslator.load(QString("qt_") + CurrentLanguage + QString(".qm"), UserConfigPath ) // need to use filename with separate directory !!
   ...
   if (!LoadConfigurationFile(USERCONFIGFILE))     // Load values from user configuration file (overwrite previously initialized values)
...
bool cApplicationConfig::LoadConfigurationFile(LoadConfigFileType TypeConfigFile) 
{
    ...
    if (IsOk) {
        ...
        if ((root.elementsByTagName("GlobalPreferences").length()>0)&&(root.elementsByTagName("GlobalPreferences").item(0).isElement()==true)) {
            ...
            if (Element.hasAttribute("YUVPassthrough")) enableYUVPassthrough = Element.attribute("YUVPassthrough") == "1";
            if (Element.hasAttribute("SmoothSVG")) enableSmoothSVG = Element.attribute("SmoothSVG") == "1";
            if (Element.hasAttribute("UseCRF")) enableCRF = Element.attribute("UseCRF") == "1";
#ifdef ENABLE_FFMPEG_CUDA
            if (Element.hasAttribute("enableCUDA")) enableCUDA = Element.attribute("enableCUDA") == "1";
            if (Element.hasAttribute("enableQSVEnc")) enableQSVEnc = Element.attribute("enableQSVEnc") == "1";
            if (Element.hasAttribute("enableCUVID")) enableCUVID = Element.attribute("enableCUVID") == "1";
            if (Element.hasAttribute("enableQSVDec")) enableQSVDec = Element.attribute("enableQSVDec") == "1";
#endif      
        }
        ...
        if ((domDocument.elementsByTagName("CacheMemory").length()>0)&&(domDocument.elementsByTagName("CacheMemory").item(0).isElement()==true)) {
            ...
            if (Element.hasAttribute("MemCacheMaxValue"))           MemCacheMaxValue=int64_t(Element.attribute("MemCacheMaxValue").toLongLong())*int64_t(1024ll*1024ll);
        }
        ...
        if ((root.elementsByTagName("EditorOptions").length()>0)&&(root.elementsByTagName("EditorOptions").item(0).isElement()==true)) {
            ...
            //if (MemCacheMaxValue>int64_t(512*1024*1024)) MemCacheMaxValue=int64_t(512*1024*1024); // must be limited to 512 MB for 32-Bit Systems
            if (MemCacheMaxValue>int64_t(2048ll*1024ll*1024ll)) MemCacheMaxValue=int64_t(2048ll*1024ll*1024ll); // must be limited to 2048 MB for 32-Bit Systems
            #endif
            ...
            if (TimelineHeight<TIMELINEMINHEIGHT) TimelineHeight=TIMELINEMINHEIGHT;
            if (TimelineHeight>TIMELINEMAXHEIGHT) TimelineHeight=TIMELINEMAXHEIGHT;
            ...
            if (Element.hasAttribute("DefaultTransitionFamilly"))   DefaultTransitionFamily    =(TRFAMILY)Element.attribute("DefaultTransitionFamilly").toInt();
        ...
        if ((root.elementsByTagName("RenderDefault").length()>0)&&(root.elementsByTagName("RenderDefault").item(0).isElement()==true)) {
            ...
            if (Element.hasAttribute("DefaultVideoCRF"))                DefaultVideoCRF = Element.attribute("DefaultVideoCRF").toInt();
        ...
        if ((root.elementsByTagName("RecentFiles").length()>0)&&(root.elementsByTagName("RecentFiles").item(0).isElement()==true)) 
        {
            ...
            while ((Element.elementsByTagName("Recent-"+QString("%1").arg(i)).length()>0)&&(root.elementsByTagName("Recent-"+QString("%1").arg(i)).item(0).isElement()==true)) 
            {
                ...
                if( !RecentFile.contains( QDir::toNativeSeparators(File) ) )
                  RecentFile.append(QDir::toNativeSeparators(File));
                i++;
            }
        }
    ...
    if (!IsOk) 
    {
        if (TypeConfigFile==USERCONFIGFILE) 
         DeviceModelList.TranslatRenderType();
    } 
    ...
    if (UpgradeConf && TypeConfigFile == USERCONFIGFILE)
       SaveConfigurationFile();
...
bool cApplicationConfig::SaveConfigurationFile() {
    ...
    Element.setAttribute("YUVPassthrough", enableYUVPassthrough ? "1" : "0");
    Element.setAttribute("SmoothSVG", enableSmoothSVG ? "1" : "0");
    Element.setAttribute("UseCRF", enableCRF ? "1" : "0");
#ifdef ENABLE_FFMPEG_CUDA
    Element.setAttribute("enableCUDA", enableCUDA ? "1" : "0");
    Element.setAttribute("enableQSVEnc", enableQSVEnc ? "1" : "0");
    Element.setAttribute("enableCUVID", enableCUVID ? "1" : "0");
    Element.setAttribute("enableQSVDec", enableQSVDec ? "1" : "0");
#endif
    root.appendChild(Element);

    Element=domDocument.createElement("CacheMemory");
    Element.setAttribute("MemCacheMaxValue",            qlonglong(MemCacheMaxValue/(1024ll*1024ll)));
    root.appendChild(Element);
    ...
    Element.setAttribute("DefaultTransitionFamilly",    DefaultTransitionFamily);
    ...
    Element.setAttribute("DefaultVideoCRF", DefaultVideoCRF);
...
void cApplicationConfig::InitValues() 
{
    ...
    TimelineHeight              = TIMELINEMINHEIGHT;        // Initial height of the timeline
    ...
    DefaultTransitionFamily    = TRANSITIONFAMILY_BASE;   // Transition family
    DefaultTransitionSubType    = 1;                        // Transition type in the family
...
QStringList cApplicationConfig::LoadBrowserFavoritesFromDabase() 
{
       ...
       (DomDocument.elementsByTagName("BrowserFavorites").item(0).isElement() == true))
    {
       ...
       for (int i = 0; i < Nbr; i++)
          if (Element.hasAttribute(QString("BrowserFavorites_%1").arg(i)))
             BrowserFavorites.append(Element.attribute(QString("BrowserFavorites_%1").arg(i)));
    }
...
void cApplicationConfig::SaveBrowserFavoritesToDabase(QStringList BrowserFavorites) 
{
   ...
   for (int i = 0; i < BrowserFavorites.count(); i++)
      Element.setAttribute(QString("BrowserFavorites_%1").arg(i), BrowserFavorites[i]);
   DomDocument.appendChild(Element);
   SettingsTable->SetIntAndTextValue("BrowserFavorites", BrowserFavorites.count(), DomDocument.toString());
}

OBJECTTYPE cApplicationConfig::getObjectType(const QString &suffix)
{
   OBJECTTYPE ObjectType = OBJECTTYPE_UNMANAGED;
   QString lowerSuffix = suffix.toLower();
   if (pAppConfig->AllowImageExtension.contains(lowerSuffix)) 
      ObjectType = OBJECTTYPE_IMAGEFILE;
   else if (pAppConfig->AllowImageVectorExtension.contains(lowerSuffix))  
      ObjectType = OBJECTTYPE_IMAGEVECTOR;
   else if (pAppConfig->AllowVideoExtension.contains(lowerSuffix))        
      ObjectType = OBJECTTYPE_VIDEOFILE;
   else if (pAppConfig->AllowMusicExtension.contains(lowerSuffix))        
...
OBJECTTYPE cApplicationConfig::getObjectType(const QFileInfo &fi, bool bCheckForThumbnail)
{
   if (fi.isDir())                                                                       
      return OBJECTTYPE_FOLDER;
   if (fi.fileName() == "folder.jpg") 
      return  OBJECTTYPE_THUMBNAIL;
   OBJECTTYPE objectType = getObjectType(fi.suffix());
   if (fi.fileName().toLower().endsWith("-poster.jpg") || fi.fileName().toLower().endsWith("-poster.png")) 
   {  
      // Special case for video xbmc poster Thumbnails
      // Search if a video with same name exist
      QDir d(fi.absolutePath());
      QString fBaseName = fi.completeBaseName();
      QString filter = fi.fileName().left(fi.fileName().toLower().indexOf("-poster."));
      QFileInfoList l = d.entryInfoList(QStringList(filter+"*"));

      //QString ToSearch = File.fileName().left(File.fileName().toLower().indexOf("-poster."));
      for (int i = 0; i < l.count(); i++)
         if ( Database->ApplicationConfig->AllowVideoExtension.contains(l.at(i).suffix().toLower()) && (l.at(i).completeBaseName() == filter) )
            objectType = OBJECTTYPE_THUMBNAIL;
   } 
   else if (bCheckForThumbnail && fi.suffix().toLower() == "jpg") 
...
//   if (ImgCache->CachePreviewImage) 
//      NewImgCache->CachePreviewImage = new QImage(*ImgCache->CachePreviewImage);
//   if (ImgCache->CacheRenderImage)  
//      NewImgCache->CacheRenderImage = new QImage(*ImgCache->CacheRenderImage);
//   NewImgCache->ByteCount = ImgCache->ByteCount;
//   ImagesCache.cacheObjects.prepend(NewImgCache);
//}
