...
void ensureEndsWith(QString &s, QChar c)
{
   if( !s.endsWith(c) )
      s.append(c);
}
...
bool IsDriveAvailable(QString Path) 
{
   ...
   if (PhysicalPath[1] == ':' && PhysicalPath[2] == '\\') 
      PhysicalPath = PhysicalPath.left(3);
   ...
   if ( (GetDriveType(Drive) == DRIVE_REMOVABLE || GetDriveType(Drive) == DRIVE_CDROM) &&
         !GetVolumeInformation(Drive,VolumeName,sizeof(VolumeName),&SerialNumber,&MaxComponent,&FileSysFlag,SysName,sizeof(SysName)) )
      return false;
   return true;
}
#endif
...
cDriveDesc::cDriveDesc(QString ThePath,QString Alias,cApplicationConfig *ApplicationConfig) 
{
   ...
   if (Alias == PersonalFolder) 
      IconDrive = ApplicationConfig->DefaultUSERIcon.GetIcon(cCustomIcon::ICON16)->copy();
   if (Label == QApplication::translate("QCustomFolderTree","Clipart")) 
   {
      Path = ClipArtFolder;
      //if (!Path.endsWith(QDir::separator())) Path=Path+QDir::separator();
      ensureEndsWith(Path, QDir::separator());
   ...
   switch (GetDriveType(Drive)) 
   {
      ...
      default             :  
         if (IconDrive.isNull()) 
            IconDrive = ApplicationConfig->DefaultHDDIcon.GetIcon(cCustomIcon::ICON16)->copy();   
      break;
   }
   if (GetVolumeInformation(Drive,VolumeName,sizeof(VolumeName),&SerialNumber,&MaxComponent,&FileSysFlag,SysName,sizeof(SysName)))
   {
      if (Label == "") 
      {
         Label = Path;
         if (QString().fromWCharArray(VolumeName)!="") 
            Label = Label+"["+QString().fromWCharArray(VolumeName)+"]";
      }
           ...
           for (int j = 0; j < Directorys.count(); j++) 
           {
              ...
              else if (Directorys[j].fileName().toLower()=="desktop.ini") 
              {
                 ...
                 if (FileIO.open(QIODevice::ReadOnly/*|QIODevice::Text*/)) 
                 {
                    ...
                    while (AllInfo!="") {
                       int j=0;
                       while ((j<AllInfo.length())&&((AllInfo[j]>=char(32))||(AllInfo[j]==QChar(9)))) j++;
                       if (j<AllInfo.length()) {
                          Line=AllInfo.left(j);
...
cDriveList::cDriveList(cApplicationConfig *TheApplicationConfig) 
{
   ...
   ensureEndsWith(ClipArtFolder, QDir::separator());
   ModelFolder = ClipArtFolder + "model";
   ClipArtFolder.append("clipart");
   //ModelFolder = ModelFolder+"model";
   CAF = ClipArtFolder;  
   //if (CAF.endsWith(QDir::separator())) 
   //   CAF = CAF.lastIndexOf(CAF.length()-1);
   MFD = ModelFolder;    
   //if (MFD.endsWith(QDir::separator())) 
   //   MFD = MFD.lastIndexOf(MFD.length()-1);
}
...
bool cDriveList::SearchDrive(QString Path) 
{
   ...
   ensureEndsWith(Path, QDir::separator());
   int i = 0;
   while (i < drives.count() && (drives[i].Path != Path && drives[i].Path + QDir::separator() != Path)) 
      i++;
   if (i < drives.count() && (drives[i].Path == Path || drives[i].Path + QDir::separator() == Path)) 
   {
      drives[i].Flag=1;
      return true;
   } 
...
{
   //AUTOTIMER(t,"UpdateDriveList");
   for (int i = 0; i < drives.count(); i++) 
      drives[i].Flag = 0;

   if (!SearchDrive(QDir::toNativeSeparators(ClipArtFolder)))    
      drives.append(cDriveDesc(QDir::toNativeSeparators(ClipArtFolder),QApplication::translate("QCustomFolderTree","Clipart"),ApplicationConfig));
   if (!SearchDrive(QDir::toNativeSeparators(QDir::homePath()))) 
      drives.append(cDriveDesc(QDir::homePath(),PersonalFolder,ApplicationConfig));
#ifdef Q_OS_WIN
   foreach(QFileInfo drive,QDir::drives()) 
   {
      ...
      if ( ( (Path.length() >= 3 && Path.at(1) == ':' && Path.at(2)=='\\' && Path.at(0) >= 'C' && Path.at(0) <= 'Z')   // If it's a drive
         ...
         if (IsDriveAvailable(Path)) 
            drives.append(cDriveDesc(Path,"",ApplicationConfig));
   }
#elif defined(Q_OS_LINUX) || defined(Q_OS_SOLARIS)

   if (!SearchDrive("/")) drives.append(cDriveDesc("/",QApplication::translate("QCustomFolderTree","System files"),ApplicationConfig));
      ...
      while (Info!="") {
         ...
         if (InfoLine.indexOf(" ")!=-1) {
            ...
            if (Device.startsWith("/dev/")) {
               ...
               if (ToAppend.Path!="/") if (!SearchDrive(ToAppend.Path)) drives.append(ToAppend);
            }
         }
      }
   }
...
{
   ensureEndsWith(FilePath, QDir::separator());
   ...
   FilePath = QDir::toNativeSeparators(FilePath);
    ...
    int i=0;
    while ((i<drives.count()) && (drives[i].Path!=FilePath) && (drives[i].Path+QDir::separator()!=FilePath)) 
      i++;
    if ((i<drives.count())&&((drives[i].Path==FilePath)||(drives[i].Path+QDir::separator()==FilePath)))
        RetIcon.addPixmap(QPixmap().fromImage(drives[i].IconDrive));
    ...
    if (RetIcon.isNull()) 
    {
        ...
        for (int j=0;j<Directorys.count();j++) if (Directorys[j].fileName().toLower()=="desktop.ini") 
        {
            ...
            //if (!FileName.endsWith(QDir::separator())) FileName=FileName+QDir::separator();
            ensureEndsWith(FileName, QDir::separator());
            FileName=FileName+Directorys[j].fileName();
            QFile   FileIO(FileName);
            QString IconFile ="";
            #ifdef Q_OS_WIN
            ...
            if (IconFile.toLower().endsWith(".jpg") || IconFile.toLower().endsWith(".png") || IconFile.toLower().endsWith(".ico")) 
               RetIcon = QIcon(IconFile);
            #ifdef Q_OS_WIN
            else 
               RetIcon = GetIconForFileOrDir(IconFile,IconIndex);
            #endif
        }
    }
    ...
    if (RetIcon.isNull()) 
    {
        //if (!FilePath.endsWith(QDir::separator())) FilePath=FilePath+QDir::separator(); already done!
        // If root item
        for (int i = 0; i < drives.count(); i++) 
         if (FilePath == drives[i].Path) 
            RetIcon = QIcon(QPixmap().fromImage(drives[i].IconDrive));
    }

    // If nothing found, use default closed folder icon
    if (RetIcon.isNull()) 
      return ApplicationConfig->DefaultFOLDERIcon.GetIcon(); 
   return RetIcon;
}
