...
#define FILTERALLOW_OBJECTTYPE_IMAGECLIPBOARD   0x0200
...
#define TIMELINEMINHEIGHT                        144                 // min heigh of montage track =(32+4)*4
#define TIMELINEMAXHEIGHT                        224                 // max heigh of montage track (min height+4x20 pixels)
...
enum eTypeComposition { eCOMPOSITIONTYPE_BACKGROUND, eCOMPOSITIONTYPE_OBJECT, eCOMPOSITIONTYPE_SHOT };
//============================================
...
enum OnOffFilter {
   eFilterEqualize  = 0x0001,
   eFilterDespeckle = 0x0002,
   eFilterGray      = 0x0004,
   eFilterNegative  = 0x0008,
   eFilterEmboss    = 0x0010,
   eFilterEdge      = 0x0020,
   eFilterAntialias = 0x0040,
   eFilterNormalize = 0x0080,
   eFilterCharcoal  = 0x0100,
   eFilterOil       = 0x0200
};
//============================================
...
public:
    ...
    bool enableYUVPassthrough;
    bool enableSmoothSVG;
    bool enableCRF;
    ...
    TRFAMILY                DefaultTransitionFamily;                    // Transition family
    int                     DefaultTransitionSubType;                   // Transition type in the family
    ...
    int                     DefaultVideoCRF;                            // Default Video CRF Value  
    ...
    cApplicationConfig() = delete;
    cApplicationConfig(cApplicationConfig const&) = delete;
    cApplicationConfig& operator=(cApplicationConfig const&) = delete;
    cApplicationConfig(cApplicationConfig&&) = delete; 
    cApplicationConfig& operator=(cApplicationConfig&&) = delete; 
    ...
    OBJECTTYPE getObjectType(const QString &suffix); // returns type obf file-object except "OBJECTTYPE_FOLDER" and "OBJECTTYPE_THUMBNAIL"
    OBJECTTYPE getObjectType(const QFileInfo &fi, bool bCheckForThumbnail = true);   // returns type obf file-object 
    // not used void                    DuplicateRessource(qlonglong *RessourceKey);    // Duplicate an image in ressource and get a new RessourceKey
};

#ifdef pAppConfig
   #undef pAppConfig
#endif
#define pAppConfig (static_cast<cApplicationConfig *>(cBaseAppConfig::pAppCfg()))
//====================================================================================================================
